...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/conditions/WiredConditionNotFurniHaveHabbo.class */
public class WiredConditionNotFurniHaveHabbo extends InteractionWiredCondition {
    ...
    /* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/conditions/WiredConditionNotFurniHaveHabbo$JsonData.class */
    ...
    public WiredConditionNotFurniHaveHabbo(ResultSet resultSet, Item item) throws SQLException {
        super(resultSet, item);
        this.items = new THashSet<>();
    }

    public WiredConditionNotFurniHaveHabbo(int i, int i2, Item item, String str, int i3, int i4) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] objArr) {
        ...
        Collection collectionValueCollection = room.getCurrentBots().valueCollection();
        Collection collectionValueCollection2 = room.getCurrentPets().valueCollection();
        return this.items.stream().noneMatch(habboItem -> {
            THashSet<RoomTile> tilesAt = room.getLayout().getTilesAt(room.getLayout().getTile(habboItem.getX(), habboItem.getY()), habboItem.getBaseItem().getWidth(), habboItem.getBaseItem().getLength(), habboItem.getRotation());
            return habbos.stream().anyMatch(habbo -> {
                return tilesAt.contains(habbo.getRoomUnit().getCurrentLocation());
            }) || collectionValueCollection.stream().anyMatch(bot -> {
                return tilesAt.contains(bot.getRoomUnit().getCurrentLocation());
            }) || collectionValueCollection2.stream().anyMatch(pet -> {
                return tilesAt.contains(pet.getRoomUnit().getCurrentLocation());
            });
        });
    }
    ...
    public String getWiredData() {
        refresh();
        return WiredHandler.getGsonBuilder().create().toJson(new JsonData((List) this.items.stream().map((v0) -> {
            return v0.getId();
    ...
    public void loadWiredData(ResultSet resultSet, Room room) throws SQLException {
        ...
        if (string.startsWith("{")) {
            Iterator<Integer> it = ((WiredConditionFurniHaveHabbo.JsonData) WiredHandler.getGsonBuilder().create().fromJson(string, WiredConditionFurniHaveHabbo.JsonData.class)).itemIds.iterator();
            while (it.hasNext()) {
