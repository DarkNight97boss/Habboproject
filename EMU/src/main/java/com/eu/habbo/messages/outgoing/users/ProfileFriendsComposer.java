...
public class ProfileFriendsComposer extends MessageComposer {
   ...
   public ProfileFriendsComposer(THashMap<Integer, THashSet<MessengerBuddy>> map, int userId) {
      this.lovers.addAll((Collection<? extends MessengerBuddy>)map.get(1));
      this.friends.addAll((Collection<? extends MessengerBuddy>)map.get(2));
      this.haters.addAll((Collection<? extends MessengerBuddy>)map.get(3));
      this.userId = userId;
   }

   public ProfileFriendsComposer(Habbo habbo) {
      try {
         for (Entry<Integer, MessengerBuddy> map : habbo.getMessenger().getFriends().entrySet()) {
