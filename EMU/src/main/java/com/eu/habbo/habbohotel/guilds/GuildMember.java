...
public class GuildMember implements Comparable {
   ...
   public GuildMembershipStatus getMembershipStatus() {
      ...
      } else if (this.rank == GuildRank.OWNER || this.rank == GuildRank.ADMIN || this.rank == GuildRank.MEMBER) {
         return GuildMembershipStatus.MEMBER;
      } else {
         return this.rank == GuildRank.REQUESTED ? GuildMembershipStatus.PENDING : GuildMembershipStatus.NOT_MEMBER;
      }
   }
}
