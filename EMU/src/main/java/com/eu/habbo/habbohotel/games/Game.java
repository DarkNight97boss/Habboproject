...
public abstract class Game implements Runnable {
      ...
      try {
         if (habbo != null) {
            ...
            synchronized (this.teams) {
               ...
               habbo.getHabboInfo().setCurrentGame((Class<? extends Game>)this.getClass());
   ...
   public void start() {
      ...
      TObjectHashIterator var3 = this.room.getRoomSpecialTypes().getItemsOfType(WiredBlob.class).iterator();
   ...
   public void onEnd() {
      ...
      if (roomOwner != null) {
         AchievementManager.progressAchievement(
            roomOwner, Emulator.getGameEnvironment().getAchievementManager().getAchievement("GameAuthorExperience"), totalPointsGained
         );
      }
      ...
      if (winningTeam != null) {
         TObjectHashIterator var8 = winningTeam.getMembers().iterator();

         while (var8.hasNext()) {
            GamePlayer player = (GamePlayer)var8.next();
         ...
         if (winningTeam.getMembers().size() > 0) {
            var8 = this.room.getRoomSpecialTypes().getItemsOfType(InteractionWiredHighscore.class).iterator();
               ...
               Emulator.getGameEnvironment()
                  ...
                  .addHighscoreData(
                     new WiredHighscoreDataEntry(
                        item.getId(),
                        winningTeam.getMembers().stream().map(m -> m.getHabbo().getHabboInfo().getId()).collect(Collectors.toList()),
         ...
         for (GameTeam team : this.teams.values()) {
            if (team != winningTeam) {
               TObjectHashIterator var18 = team.getMembers().iterator();

               while (var18.hasNext()) {
                  GamePlayer player = (GamePlayer)var18.next();
               ...
               if (team.getMembers().size() > 0 && team.getTotalScore() > 0) {
                  var18 = this.room.getRoomSpecialTypes().getItemsOfType(InteractionWiredHighscore.class).iterator();

                  while (var18.hasNext()) {
                     HabboItem item = (HabboItem)var18.next();
                     Emulator.getGameEnvironment()
                        .getItemManager()
                        .getHighscoreManager()
                        .addHighscoreData(
                           new WiredHighscoreDataEntry(
                              item.getId(),
                              team.getMembers().stream().map(m -> m.getHabbo().getHabboInfo().getId()).collect(Collectors.toList()),
                              team.getTotalScore(),
                              false,
                              Emulator.getIntUnixTimestamp()
                           )
      ...
      TObjectHashIterator var11 = this.room.getRoomSpecialTypes().getItemsOfType(InteractionWiredHighscore.class).iterator();

      while (var11.hasNext()) {
         HabboItem item = (HabboItem)var11.next();
         ((InteractionWiredHighscore)item).reloadData();
         this.room.updateItem(item);
      }

      var11 = this.room.getRoomSpecialTypes().getItemsOfType(WiredBlob.class).iterator();
   ...
   public void stop() {
      ...
      TObjectHashIterator gameStoppedEvent = this.room.getFloorItems().iterator();

      while (gameStoppedEvent.hasNext()) {
         HabboItem timer = (HabboItem)gameStoppedEvent.next();
         if (timer instanceof InteractionGameTimer && ((InteractionGameTimer)timer).isRunning()) {
            gamesActive = true;
         }
      }
      ...
      if (!gamesActive) {
         if (Emulator.getPluginManager().isRegistered(GameStoppedEvent.class, true)) {
            Event gameStoppedEventx = new GameStoppedEvent(this);
            Emulator.getPluginManager().fireEvent(gameStoppedEventx);
         }
      }
   }
   ...
   private void saveScores() {
      if (this.room != null) {
         ...
         for (Entry<GameTeamColors, GameTeam> teamEntry : teamsCopy.entrySet()) {
   ...
   public GameTeam getTeam(GameTeamColors teamColor) {
      synchronized (this.teams) {
         return (GameTeam)this.teams.get(teamColor);
      }
   }
