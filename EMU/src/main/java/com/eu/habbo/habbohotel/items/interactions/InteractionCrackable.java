   ...
   public void serializeExtradata(ServerMessage serverMessage) {
      ...
      serverMessage.appendString(
         Emulator.getGameEnvironment()
               .getItemManager()
               .calculateCrackState(
                  Integer.valueOf(this.getExtradata()),
                  Emulator.getGameEnvironment().getItemManager().getCrackableCount(this.getBaseItem().getId()),
                  this.getBaseItem()
               )
            + ""
      );
            ...
            if (this.getRoomId() != 0) {
               if (!this.cracked) {
                  if (this.userRequiredToBeAdjacent()
                     && client.getHabbo().getRoomUnit().getCurrentLocation().distance(room.getLayout().getTile(this.getX(), this.getY())) > 1.5) {
                     client.getHabbo()
                        .getRoomUnit()
                        .setGoalLocation(
                           room.getLayout()
                              .getTileInFront(
                                 ...
                                 Rotation.Calculate(client.getHabbo().getRoomUnit().getX(), client.getHabbo().getRoomUnit().getY(), this.getX(), this.getY())
                              )
                        );
                  } else {
                     if (this.getExtradata().length() == 0) {
                     ...
                     if (this.getBaseItem().getEffectF() <= 0
                        || !client.getHabbo().getHabboInfo().getGender().equals(HabboGender.F)
                        || this.getBaseItem().getEffectF() != client.getHabbo().getRoomUnit().getEffectId()) {
                        if (this.getBaseItem().getEffectM() <= 0
                           || !client.getHabbo().getHabboInfo().getGender().equals(HabboGender.M)
                           || this.getBaseItem().getEffectM() != client.getHabbo().getRoomUnit().getEffectId()) {
            ...
            if (rewardData != null) {
               ...
               if (!rewardData.achievementTick.isEmpty()) {
                  AchievementManager.progressAchievement(
                     habbo, Emulator.getGameEnvironment().getAchievementManager().getAchievement(rewardData.achievementTick)
                  );
               }

               if (!this.cracked && this.ticks == Emulator.getGameEnvironment().getItemManager().getCrackableCount(this.getBaseItem().getId())) {
                  this.cracked = true;
                  Emulator.getThreading().run(new CrackableExplode(room, this, habbo, !this.placeInRoom(), this.getX(), this.getY()), 1500L);
                  if (!rewardData.achievementCracked.isEmpty()) {
                     AchievementManager.progressAchievement(
                        habbo, Emulator.getGameEnvironment().getAchievementManager().getAchievement(rewardData.achievementCracked)
                     );
                  }
                  ...
                  if (rewardData.subscriptionType != null && rewardData.subscriptionDuration > 0) {
                     switch (rewardData.subscriptionType) {
                        case HABBO_CLUB:
                           habbo.getHabboStats().createSubscription("HABBO_CLUB", rewardData.subscriptionDuration * 86400);
                           break;
                        case BUILDERS_CLUB:
