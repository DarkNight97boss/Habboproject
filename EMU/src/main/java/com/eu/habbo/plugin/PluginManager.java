...
public class PluginManager {
   ...
   public static void globalOnConfigurationUpdated(EmulatorConfigUpdatedEvent event) {
      ...
      DiscountComposer.ADDITIONAL_DISCOUNT_THRESHOLDS = Arrays.stream(Emulator.getConfig().getValue("discount.additional.thresholds", "40;99").split(";"))
      ...
      HabboManager.WELCOME_MESSAGE = Emulator.getConfig()
      ...
      ChangeNameCheckUsernameEvent.VALID_CHARACTERS = Emulator.getConfig()
         .getValue("allowed.username.characters", "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_-=!?@:,.");
      ...
      try {
         SubscriptionHabboClub.HC_PAYDAY_NEXT_DATE = (int)(
            Emulator.stringToDate(Emulator.getConfig().getValue("subscriptions.hc.payday.next_date")).getTime() / 1000L
         );
      } catch (Exception e) {
      ...
      for (String streak : Emulator.getConfig().getValue("subscriptions.hc.payday.streak", "7=5;30=10;60=15;90=20;180=25;365=30").split(Pattern.quote(";"))) {
         if (streak.contains("=")) {
            SubscriptionHabboClub.HC_PAYDAY_STREAK
               .put(Integer.parseInt(streak.split(Pattern.quote("="))[0]), Integer.parseInt(streak.split(Pattern.quote("="))[1]));
         }
      }
      ...
      if (Emulator.isReady) {
         GiftConfigurationComposer.BOX_TYPES = Arrays.stream(Emulator.getConfig().getValue("hotel.gifts.box_types").split(","))
            .mapToInt(Integer::parseInt)
            .boxed()
            .collect(Collectors.toList());
         GiftConfigurationComposer.RIBBON_TYPES = Arrays.stream(Emulator.getConfig().getValue("hotel.gifts.ribbon_types").split(","))
            .mapToInt(Integer::parseInt)
            .boxed()
            .collect(Collectors.toList());
   ...
   public void loadPlugins() {
      ...
      for (File file : Objects.requireNonNull(loc.listFiles(filex -> filex.getPath().toLowerCase().endsWith(".jar")))) {
         try {
            URLClassLoader urlClassLoader = URLClassLoader.newInstance(new URL[]{file.toURI().toURL()});
            InputStream stream = urlClassLoader.getResourceAsStream("plugin.json");
            if (stream == null) {
            ...
            if (stream.read(content) > 0) {
               ...
               HabboPluginConfiguration pluginConfigurtion = (HabboPluginConfiguration)gson.fromJson(body, HabboPluginConfiguration.class);
      ...
      synchronized (plugin.registeredEvents) {
         ...
         for (Method method : methods) {
            if (method.getAnnotation(EventHandler.class) != null
               ...
               ((THashSet)plugin.registeredEvents.get(eventClass.asSubclass(Event.class))).add(method);
            }
         }
      }
   }
   ...
   public <T extends Event> T fireEvent(T event) {
      TObjectHashIterator iterator = this.methods.iterator();
      ...
      while (iterator.hasNext()) {
         ...
         if (method.getParameterTypes().length == 1 && method.getParameterTypes()[0].isAssignableFrom(event.getClass())) {
            try {
               ...
               LOGGER.error(
                  "Could not pass default event {} to {}: {}!", new Object[]{event.getClass().getName(), method.getClass().getName(), method.getName()}
               );
               LOGGER.error("Caught exception", e);
            }
         }
      ...
      while (iterator.hasNext()) {
         try {
            HabboPlugin plugin = (HabboPlugin)iterator.next();
            if (plugin != null) {
               THashSet<Method> methods = (THashSet<Method>)plugin.registeredEvents.get(event.getClass().asSubclass(Event.class));
               if (methods != null) {
                  TObjectHashIterator var5 = methods.iterator();
   ...
   public boolean isRegistered(Class<? extends Event> clazz, boolean pluginsOnly) {
      ...
      if (!pluginsOnly) {
         TObjectHashIterator var7 = this.methods.iterator();
   ...
   private void disposePlugins() {
      ...
      while (iterator.hasNext()) {
         try {
            HabboPlugin p = (HabboPlugin)iterator.next();
            if (p != null) {
               try {
                  p.onDisable();
                  p.stream.close();
