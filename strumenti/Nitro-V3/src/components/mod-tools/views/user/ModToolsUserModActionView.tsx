...
// Action labels are localized lazily inside the component (see useMemo below)
// because LocalizeText reads from the renderer's LocalizationManager which is
// only populated after the externalTexts URL fetch resolves — building this
// list at module import time would freeze in English on first paint.
const buildModActionDefinitions = (): ModActionDefinition[] => [
    new ModActionDefinition(1, LocalizeText('modtools.user.modaction.def.alert'), ModActionDefinition.ALERT, 1, 0),
    new ModActionDefinition(2, LocalizeText('modtools.user.modaction.def.mute1h'), ModActionDefinition.MUTE, 2, 0),
    new ModActionDefinition(3, LocalizeText('modtools.user.modaction.def.ban18h'), ModActionDefinition.BAN, 3, 0),
    new ModActionDefinition(4, LocalizeText('modtools.user.modaction.def.ban7d'), ModActionDefinition.BAN, 4, 0),
    new ModActionDefinition(5, LocalizeText('modtools.user.modaction.def.ban30d_1'), ModActionDefinition.BAN, 5, 0),
    new ModActionDefinition(7, LocalizeText('modtools.user.modaction.def.ban30d_2'), ModActionDefinition.BAN, 7, 0),
    new ModActionDefinition(6, LocalizeText('modtools.user.modaction.def.ban100y'), ModActionDefinition.BAN, 6, 0),
    new ModActionDefinition(106, LocalizeText('modtools.user.modaction.def.ban_avatar_100y'), ModActionDefinition.BAN, 6, 0),
    new ModActionDefinition(101, LocalizeText('modtools.user.modaction.def.kick'), ModActionDefinition.KICK, 0, 0),
    new ModActionDefinition(102, LocalizeText('modtools.user.modaction.def.tradelock_1w'), ModActionDefinition.TRADE_LOCK, 0, 168),
    new ModActionDefinition(104, LocalizeText('modtools.user.modaction.def.tradelock_perm'), ModActionDefinition.TRADE_LOCK, 0, 876000),
    new ModActionDefinition(105, LocalizeText('modtools.user.modaction.def.message'), ModActionDefinition.MESSAGE, 0, 0),
];
...
export const ModToolsUserModActionView: FC<ModToolsUserModActionViewProps> = props =>
{
    ...
    const MOD_ACTION_DEFINITIONS = useMemo(() => buildModActionDefinitions(), []);
    ...
    const sendAlert = (m: string) => simpleAlert(m, NotificationAlertType.DEFAULT, null, null, LocalizeText('modtools.generic.error.title'));
