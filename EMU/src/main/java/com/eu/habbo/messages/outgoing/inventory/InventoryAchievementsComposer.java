...
public class InventoryAchievementsComposer extends MessageComposer {
   ...
   protected ServerMessage composeInternal() {
      ...
      synchronized (Emulator.getGameEnvironment().getAchievementManager().getAchievements()) {
         ...
         achievements.forEachValue(
            new TObjectProcedure<Achievement>() {
               public boolean execute(Achievement achievement) {
                  InventoryAchievementsComposer.this.response
                     .appendString(achievement.name.startsWith("ACH_") ? achievement.name.replace("ACH_", "") : achievement.name);
