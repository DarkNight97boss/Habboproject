...
public class InteractionPuzzleBox extends HabboItem {
   ...
   public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
      ...
      if (rotation == null) {
         ...
         if (nearestTile != null) {
            client.getHabbo().getRoomUnit().setGoalLocation(nearestTile);
         }
      } else {
         ...
         if (tile != null && tile.getState() != RoomTileState.INVALID && !room.hasHabbosAt(tile.x, tile.y)) {
            if (boxLocation.equals(room.getLayout().getTileInFront(client.getHabbo().getRoomUnit().getCurrentLocation(), rotation.getValue()))) {
               ...
               if (item == null || room.getTopItemAt(tile.x, tile.y).getBaseItem().allowStack()) {
                  this.setZ(room.getStackHeight(tile.x, tile.y, false));
                  this.needsUpdate(true);
                  room.updateItem(this);
                  room.scheduledComposers.add(new FloorItemOnRollerComposer(this, null, tile, 0.0, room).compose());
                  room.scheduledTasks.add((Runnable)() -> {
                     client.getHabbo().getRoomUnit().setGoalLocation(boxLocation);
                     room.scheduledTasks.add((Runnable)() -> client.getHabbo().getRoomUnit().setGoalLocation(boxLocation));
                  });
                  this.needsUpdate(true);
               }
            }
