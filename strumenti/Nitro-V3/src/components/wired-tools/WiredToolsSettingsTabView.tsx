...
const buildInspectOptions = (): RoomAccessOption[] => [
    { bit: WIRED_ACCESS_EVERYONE, label: LocalizeText('wired.tools.access.everyone') },
    { bit: WIRED_ACCESS_USERS_WITH_RIGHTS, label: LocalizeText('wired.tools.access.users_with_rights') },
    { bit: WIRED_ACCESS_GROUP_MEMBERS, label: LocalizeText('wired.tools.access.group_members') },
    { bit: WIRED_ACCESS_GROUP_ADMINS, label: LocalizeText('wired.tools.access.group_admins') }
];
...
export const WiredToolsSettingsTabView: FC<{}> = () =>
{
    ...
    return (
        <div className="p-3 min-h-[360px] flex flex-col gap-3">
            <Text bold>{ LocalizeText('wired.tools.settings.room_settings') }:</Text>
            <div className="grid grid-cols-2 gap-3">
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>{ LocalizeText('wired.tools.settings.who_modify') }:</Text>
                    { modifyOptions.map(option => renderAccessOption(option, roomSettings.modifyMask, updateModifyMask)) }
                </div>
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>{ LocalizeText('wired.tools.settings.who_inspect') }:</Text>
                    { inspectOptions.map(option => renderAccessOption(option, roomSettings.inspectMask, updateInspectMask)) }
                </div>
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>{ LocalizeText('wired.tools.settings.timezone') }:</Text>
                    <select
                ...
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>{ LocalizeText('wired.tools.settings.room_state') }:</Text>
                    <div className="flex gap-2">
                        <Button
                            ...
                            { LocalizeText('wired.tools.settings.reload') }
                        </Button>
                        <Button classNames={ [ 'flex-1' ] } disabled variant="danger">
                            { LocalizeText('wired.tools.settings.rollback') }
                        </Button>
                    </div>
                </div>
            </div>
            <div className="flex flex-col gap-2">
                <Text bold>{ LocalizeText('wired.tools.settings.account_preferences') }:</Text>
                <div className="rounded bg-[#dfddd7] p-3 flex flex-col gap-2">
                    <Text bold small>{ LocalizeText('wired.tools.settings.general') }:</Text>
                    <label className="flex items-center gap-2 text-[12px] text-[#222]">
                        ...
                        <span>{ LocalizeText('wired.tools.settings.show_toolbar_button') }</span>
                    </label>
                    <label className="flex items-center gap-2 text-[12px] text-[#222]">
                        <input
                        ...
                        <span>{ LocalizeText('wired.tools.settings.show_inspect_button') }</span>
                    </label>
                    <label className="flex items-center gap-2 text-[12px] text-[#222]">
                        <input
                        ...
                        <span>{ LocalizeText('wired.tools.settings.show_all_notifications') }</span>
                    </label>
                </div>
            </div>
        </div>
