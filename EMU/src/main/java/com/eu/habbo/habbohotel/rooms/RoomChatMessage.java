...
public class RoomChatMessage implements Runnable, ISerialize, DatabaseLoggable {
   ...
   public RoomChatMessage(MessageHandler message) {
      if (message.packet.getMessageId() == 1543) {
   ...
   public void serialize(ServerMessage message) {
      ...
      if (!this.getBubble().getPermission().isEmpty() && this.habbo != null && !this.habbo.hasPermission(this.getBubble().getPermission())) {
         this.bubble = RoomChatMessageBubbles.NORMAL;
      }
   ...
   public void filter() {
      ...
      if (Emulator.getConfig().getBoolean("hotel.wordfilter.enabled")
         ...
         && !this.habbo.hasPermission(Permission.ACC_CHAT_NO_FILTER)) {
   ...
   public String getQuery() {
      return "INSERT INTO chatlogs_room (user_from_id, user_to_id, message, timestamp, room_id) VALUES (?, ?, ?, ?, ?)";
   }
