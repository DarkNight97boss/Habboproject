...
public abstract class HabboItem implements Runnable, IEventTriggers {
   ...
   private static Class[] TOGGLING_INTERACTIONS = new Class[]{
      InteractionGameTimer.class, InteractionWired.class, InteractionWiredHighscore.class, InteractionMultiHeight.class
   };
   private int id;
   private int userId;
   private int roomId;
      ...
      try {
         ...
         serverMessage.appendString(
            this.getBaseItem().getInteractionType().getType() == InteractionTrophy.class
                  ...
                  || this.getBaseItem().getName().toLowerCase().equals("gnome_box")
               ? "1.0"
               : (!this.getBaseItem().allowWalk() && (!this.getBaseItem().allowSit() || this.roomId == 0) ? "" : Item.getCurrentHeight(this) + "")
         );
      } catch (Exception e) {
            ...
            if (this.needsDelete) {
               ...
               try {
                  ...
                  try {
                     ...
                     statement.setDouble(6, Math.max(-9999.0, Math.min(9999.0, Math.round(this.z * Math.pow(10.0, 6.0)) / Math.pow(10.0, 6.0))));
   ...
   public abstract boolean canWalkOn(RoomUnit var1, Room var2, Object[] var3);
   ...
   public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
      if (client != null && this.getBaseItem().getType() == FurnitureType.FLOOR) {
         ...
         if (this.getBaseItem().getStateCount() > 1 && !(this instanceof InteractionDice)
            || Arrays.asList(TOGGLING_INTERACTIONS).contains(this.getClass())
   ...
   public abstract void onWalk(RoomUnit var1, Room var2, Object[] var3) throws Exception;
   ...
   public void onPickUp(Room room) {
      if (this.getBaseItem().getEffectF() > 0 || this.getBaseItem().getEffectM() > 0) {
         ...
         TObjectHashIterator var5 = room.getHabbosOnItem(this).iterator();

         while (var5.hasNext()) {
            Habbo habbo = (Habbo)var5.next();
         ...
         var5 = room.getBotsAt(room.getLayout().getTile(this.getX(), this.getY())).iterator();
   ...
   public void onMove(Room room, RoomTile oldLocation, RoomTile newLocation) {
      if (this.getBaseItem().getEffectF() > 0 || this.getBaseItem().getEffectM() > 0) {
         ...
         TObjectHashIterator var11 = room.getLayout()
            .getTilesAt(oldLocation, this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation())
            .iterator();
         ...
         var11 = room.getLayout().getTilesAt(oldLocation, this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation()).iterator();
   ...
   public Rectangle getRectangle() {
      return RoomLayout.getRectangle(this.getX(), this.getY(), this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation());
   }
