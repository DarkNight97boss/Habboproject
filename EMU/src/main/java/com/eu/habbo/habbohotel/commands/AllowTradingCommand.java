   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      ...
      if (!option.equalsIgnoreCase(Emulator.getTexts().getValue("generic.yes")) && !option.equalsIgnoreCase(Emulator.getTexts().getValue("generic.no"))) {
         gameClient.getHabbo()
            .whisper(
               Emulator.getTexts()
                  .getValue("commands.error.cmd_allow_trading.incorrect_setting")
                  .replace("%enabled%", Emulator.getTexts().getValue("generic.yes"))
                  .replace("%disabled%", Emulator.getTexts().getValue("generic.no"))
            );
      } else {
         boolean enabled = option.equalsIgnoreCase(Emulator.getTexts().getValue("generic.yes"));
         ...
         if (habbo != null) {
            if (!enabled) {
               ...
               try {
                  PreparedStatement statement = connection.prepareStatement(
                     "UPDATE users_settings SET tradelock_amount = tradelock_amount + 1 WHERE user_id = ?"
                  );
            ...
            gameClient.getHabbo()
               .whisper(
                  Emulator.getTexts().getValue("commands.succes.cmd_allow_trading." + (enabled ? "enabled" : "disabled")).replace("%username%", params[1])
               );
         ...
         try {
            PreparedStatement statement = connection.prepareStatement(
               "UPDATE users_settings INNER JOIN users ON users.id = users_settings.user_id SET can_trade = ?, tradelock_amount = tradelock_amount + ? WHERE users.username LIKE ?"
            );
         ...
         gameClient.getHabbo()
            .whisper(Emulator.getTexts().getValue("commands.succes.cmd_allow_trading." + (enabled ? "enabled" : "disabled")).replace("%username%", params[1]));
      }
