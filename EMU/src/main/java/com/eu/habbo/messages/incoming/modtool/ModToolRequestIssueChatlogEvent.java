         ...
         if (issue != null) {
            ...
            if (issue.type == ModToolTicketType.IM) {
               ...
               if (issue.commentId == -1) {
                  ...
                  if (thread != null) {
                     chatlog = thread.getComments()
                        .stream()
                        .map(
                           c -> new ModToolChatLog(
                              c.getCreatedAt(), c.getHabbo().getHabboInfo().getId(), c.getHabbo().getHabboInfo().getUsername(), c.getMessage()
                           )
                        )
                  ...
                  if (thread != null) {
                     chatlog = thread.getComments()
                        ...
                        .map(
                           c -> new ModToolChatLog(
                              c.getCreatedAt(),
                              c.getHabbo().getHabboInfo().getId(),
                              c.getHabbo().getHabboInfo().getUsername(),
                              c.getMessage(),
                              c.getCommentId() == issue.commentId
                           )
                        )
                        .collect(Collectors.toList());
                  }
