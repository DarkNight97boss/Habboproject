...
public class MonsterplantPet extends Pet implements IPetLook {
   ...
   public static final Map<Integer, Pair<String, Integer>> bodyRarity = new LinkedHashMap<Integer, Pair<String, Integer>>() {
      {
         ...
         this.put(12, new Pair("Snozzle", 5));
      }
   };
   ...
   public String getName() {
      ...
      if (colorRarity.containsKey(this.hue)) {
         name = (String)colorRarity.get(this.hue).getKey();
      }

      if (bodyRarity.containsKey(this.type)) {
         name = name + " " + (String)bodyRarity.get(this.type).getKey();
      }
         ...
         if (this.isDead()) {
            ...
            if (!this.hasDied) {
               AchievementManager.progressAchievement(
                  ...
                  Emulator.getGameEnvironment().getAchievementManager().getAchievement("MonsterPlantGardenOfDeath")
               );
               this.hasDied = true;
   ...
   public int getRarity() {
      return bodyRarity.containsKey(this.type) && colorRarity.containsKey(this.hue)
         ? (Integer)bodyRarity.get(this.type).getValue() + (Integer)colorRarity.get(this.hue).getValue()
         : 0;
   }

   @Override
   public String getLook() {
      return "16 0 FFFFFF 5 0 -1 10 1 "
         + this.type
         + " "
         + this.hue
         + " 2 "
   ...
   public int remainingGrowTime() {
      return this.growthStage == 7 ? 0 : Math.max(0, growTime - (Emulator.getIntUnixTimestamp() - this.created));
   }
      ...
      if (this.canBreed && pet.canBreed) {
         ...
         if (seedBase != null) {
            if (ownerOne != null) {
               ...
               HabboItem seed = Emulator.getGameEnvironment().getItemManager().createItem(ownerOne.getHabboInfo().getId(), seedBase, 0, 0, "");
            ...
            if (ownerTwo != null) {
               ...
               HabboItem seed = Emulator.getGameEnvironment().getItemManager().createItem(ownerTwo.getHabboInfo().getId(), seedBase, 0, 0, "");
   ...
   private boolean mayScratch() {
      ...
      float energyMax = this.getMaxEnergy();
   ...
   public int getEnergy() {
      return this.isDead() ? 100 : this.deathTimestamp - Emulator.getIntUnixTimestamp();
   }
   ...
   public synchronized void scratched(Habbo habbo) {
      if (this.mayScratch()) {
         ...
         this.setDeathTimestamp(Emulator.getIntUnixTimestamp() + timeToLive);
