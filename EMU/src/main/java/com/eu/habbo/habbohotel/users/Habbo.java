...
public class Habbo implements Runnable {
   ...
   public boolean connect() {
      ...
      LOGGER.info("{} logged in from IP {} using proxyserver {}", new Object[]{this.habboInfo.getUsername(), this.habboInfo.getIpLogin(), ProxyIP});
      return true;
   }
   ...
   public void talk(String message, RoomChatMessageBubbles bubble) {
      if (this.getRoomUnit().isInRoom()) {
         this.getHabboInfo()
            .getCurrentRoom()
            .sendComposer(new RoomUserTalkComposer(new RoomChatMessage(message, this.client.getHabbo().getRoomUnit(), bubble)).compose());
      }
   }
   ...
   public void shout(String message, RoomChatMessageBubbles bubble) {
      if (this.getRoomUnit().isInRoom()) {
         this.getHabboInfo()
            .getCurrentRoom()
            .sendComposer(new RoomUserShoutComposer(new RoomChatMessage(message, this.client.getHabbo().getRoomUnit(), bubble)).compose());
      }
   }
   ...
   public void clearCaches() {
      ...
      for (Entry<Integer, List<Integer>> ltdLog : this.habboStats.ltdPurchaseLog.entrySet()) {
         for (Integer time : ltdLog.getValue()) {
            if (time > timestamp) {
               ...
               ((List)newLog.get(ltdLog.getKey())).add(time);
            }
         }
      }
   ...
   public void respect(Habbo target) {
      if (target != null && target != this.client.getHabbo()) {
         ...
         this.client
            ...
            .getHabboInfo()
            .getCurrentRoom()
            .sendComposer(new RoomUserActionComposer(this.client.getHabbo().getRoomUnit(), RoomUserAction.THUMB_UP).compose());
