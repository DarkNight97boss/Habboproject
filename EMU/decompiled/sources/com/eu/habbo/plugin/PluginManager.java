...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/plugin/PluginManager.class */
public class PluginManager {
    ...
    @EventHandler
    public static void globalOnConfigurationUpdated(EmulatorConfigUpdatedEvent emulatorConfigUpdatedEvent) {
        ...
        RoomLayout.MAXIMUM_STEP_HEIGHT = Emulator.getConfig().getDouble("pathfinder.step.maximum.height", Double.valueOf(1.1d));
        ...
        Room.MAXIMUM_POSTITNOTES = Emulator.getConfig().getInt("hotel.room.stickies.max", Integer.valueOf(RentableSpaceInfoComposer.NOT_ENOUGH_CREDITS));
        ...
        String[] strArrSplit = Emulator.getConfig().getValue("commands.cmd_chatcolor.banned_numbers").split(";");
        RoomChatMessage.BANNED_BUBBLES = new int[strArrSplit.length];
        for (int i = 0; i < RoomChatMessage.BANNED_BUBBLES.length; i++) {
            try {
                RoomChatMessage.BANNED_BUBBLES[i] = Integer.valueOf(strArrSplit[i]).intValue();
            } catch (Exception e) {
        ...
        SubscriptionHabboClub.HC_PAYDAY_KICKBACK_PERCENTAGE = Double.valueOf(((double) Emulator.getConfig().getInt("subscriptions.hc.payday.percentage", 10)) / 100.0d);
        ...
        for (String str : Emulator.getConfig().getValue("subscriptions.hc.payday.streak", "7=5;30=10;60=15;90=20;180=25;365=30").split(Pattern.quote(";"))) {
            if (str.contains("=")) {
                SubscriptionHabboClub.HC_PAYDAY_STREAK.put(Integer.valueOf(Integer.parseInt(str.split(Pattern.quote("="))[0])), Integer.valueOf(Integer.parseInt(str.split(Pattern.quote("="))[1])));
            }
        }
        ...
        String value = Emulator.getConfig().getValue("gamedata.figuredata.url");
        if (!ClothingValidationManager.FIGUREDATA_URL.equals(value)) {
            ClothingValidationManager.FIGUREDATA_URL = value;
            ClothingValidationManager.reloadFiguredata(value);
        }
        if (value.isEmpty()) {
        ...
        for (String str2 : Emulator.getConfig().getValue("navigator.eventcategories", Emulator.PREVIEW).split(";")) {
            try {
                NewNavigatorEventCategoriesComposer.CATEGORIES.add(new EventCategory(str2));
            } catch (Exception e3) {
                e3.printStackTrace();
            }
        }
        if (Emulator.isReady) {
            GiftConfigurationComposer.BOX_TYPES = (List) Arrays.stream(Emulator.getConfig().getValue("hotel.gifts.box_types").split(",")).mapToInt(Integer::parseInt).boxed().collect(Collectors.toList());
            GiftConfigurationComposer.RIBBON_TYPES = (List) Arrays.stream(Emulator.getConfig().getValue("hotel.gifts.ribbon_types").split(",")).mapToInt(Integer::parseInt).boxed().collect(Collectors.toList());
    ...
    public void loadPlugins() {
        URLClassLoader uRLClassLoaderNewInstance;
        ...
        for (File file2 : (File[]) Objects.requireNonNull(file.listFiles(file3 -> {
            return file3.getPath().toLowerCase().endsWith(".jar");
        }))) {
            try {
                uRLClassLoaderNewInstance = URLClassLoader.newInstance(new URL[]{file2.toURI().toURL()});
                resourceAsStream = uRLClassLoaderNewInstance.getResourceAsStream("plugin.json");
            } catch (Exception e) {
                LOGGER.error("Caught exception", e);
            }
            if (resourceAsStream == null) {
                throw new RuntimeException("Invalid Jar! Missing plugin.json in: " + file2.getName());
            }
            byte[] bArr = new byte[resourceAsStream.available()];
            if (resourceAsStream.read(bArr) > 0) {
                HabboPluginConfiguration habboPluginConfiguration = (HabboPluginConfiguration) new GsonBuilder().create().fromJson(new String(bArr), HabboPluginConfiguration.class);
                try {
                    HabboPlugin habboPlugin = (HabboPlugin) uRLClassLoaderNewInstance.loadClass(habboPluginConfiguration.main).asSubclass(HabboPlugin.class).getConstructor(new Class[0]).newInstance(new Object[0]);
                    habboPlugin.configuration = habboPluginConfiguration;
                    habboPlugin.classLoader = uRLClassLoaderNewInstance;
                    habboPlugin.stream = resourceAsStream;
                    this.plugins.add(habboPlugin);
                    habboPlugin.onEnable();
                } catch (Exception e2) {
                    LOGGER.error("Could not load plugin {}!", habboPluginConfiguration.name);
    ...
    public void registerEvents(HabboPlugin habboPlugin, EventListener eventListener) {
        synchronized (habboPlugin.registeredEvents) {
            for (Method method : eventListener.getClass().getMethods()) {
                if (method.getAnnotation(EventHandler.class) != null && method.getParameterTypes().length == 1 && Event.class.isAssignableFrom(method.getParameterTypes()[0])) {
                    ...
                    if (!habboPlugin.registeredEvents.containsKey(cls.asSubclass(Event.class))) {
                        habboPlugin.registeredEvents.put(cls.asSubclass(Event.class), new THashSet());
                    }
                    ((THashSet) habboPlugin.registeredEvents.get(cls.asSubclass(Event.class))).add(method);
                }
            }
        }
    }
    ...
    public <T extends Event> T fireEvent(T t) {
        THashSet tHashSet;
        TObjectHashIterator it = this.methods.iterator();
        while (it.hasNext()) {
            Method method = (Method) it.next();
            if (method.getParameterTypes().length == 1 && method.getParameterTypes()[0].isAssignableFrom(t.getClass())) {
                try {
                    method.invoke(null, t);
                } catch (Exception e) {
                    LOGGER.error("Could not pass default event {} to {}: {}!", new Object[]{t.getClass().getName(), method.getClass().getName(), method.getName()});
                    LOGGER.error("Caught exception", e);
                }
            }
        }
        TObjectHashIterator it2 = this.plugins.iterator();
        while (it2.hasNext()) {
            try {
                HabboPlugin habboPlugin = (HabboPlugin) it2.next();
                if (habboPlugin != null && (tHashSet = (THashSet) habboPlugin.registeredEvents.get(t.getClass().asSubclass(Event.class))) != null) {
                    TObjectHashIterator it3 = tHashSet.iterator();
                    while (it3.hasNext()) {
                        try {
                            ((Method) it3.next()).invoke(habboPlugin, t);
                        } catch (Exception e2) {
                            LOGGER.error("Could not pass event {} to {}", t.getClass().getName(), habboPlugin.configuration.name);
                            LOGGER.error("Caught exception", e2);
                        }
                    }
                }
    ...
    public boolean isRegistered(Class<? extends Event> cls, boolean z) {
        TObjectHashIterator it = this.plugins.iterator();
        while (it.hasNext()) {
            try {
                if (((HabboPlugin) it.next()).isRegistered(cls)) {
                    return true;
                }
        ...
        TObjectHashIterator it2 = this.methods.iterator();
        while (it2.hasNext()) {
    ...
    private void disposePlugins() {
        ...
        while (it.hasNext()) {
            try {
                ...
                if (habboPlugin != null) {
                    try {
                        habboPlugin.onDisable();
                        habboPlugin.stream.close();
                        habboPlugin.classLoader.close();
                    } catch (IOException e) {
                        LOGGER.error("Caught exception", e);
                    } catch (Exception e2) {
                        LOGGER.error("Failed to disable {} because of an exception.", habboPlugin.configuration.name, e2);
                    }
                }
            } catch (NoSuchElementException e3) {
            }
    ...
    public void reload() {
        ...
        this.methods.clear();
        loadPlugins();
        LOGGER.info("Plugin Manager -> Loaded! " + this.plugins.size() + " plugins! (" + (System.currentTimeMillis() - jCurrentTimeMillis) + " MS)");
