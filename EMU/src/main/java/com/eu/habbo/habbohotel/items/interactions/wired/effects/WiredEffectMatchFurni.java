...
public class WiredEffectMatchFurni extends InteractionWiredEffect implements InteractionWiredMatchFurniSettings {
   ...
   public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
      ...
      TObjectHashIterator var4 = this.settings.iterator();

      while (var4.hasNext()) {
         WiredMatchFurniSetting setting = (WiredMatchFurniSetting)var4.next();
         HabboItem item = room.getHabboItem(setting.item_id);
         if (item != null) {
            if (this.state
               && this.checkForWiredResetPermission
               && item.allowWiredResetState()
               && !setting.state.equals(" ")
               && !item.getExtradata().equals(setting.state)) {
            ...
            if (this.direction && !this.position) {
               ...
               if (newLocation != null
                  ...
                  && room.furnitureFitsAt(newLocation, item, newRotation, true) == FurnitureMovementError.NONE
                  && room.moveFurniTo(item, newLocation, newRotation, null, !slideAnimation) == FurnitureMovementError.NONE
                  && slideAnimation) {
   ...
   public String getWiredData() {
      ...
      return WiredHandler.getGsonBuilder()
         .create()
         .toJson(new WiredEffectMatchFurni.JsonData(this.state, this.direction, this.position, new ArrayList<>(this.settings), this.getDelay()));
   }
   ...
   public void loadWiredData(ResultSet set, Room room) throws SQLException {
      ...
      if (wiredData.startsWith("{")) {
         WiredEffectMatchFurni.JsonData data = (WiredEffectMatchFurni.JsonData)WiredHandler.getGsonBuilder()
            .create()
            .fromJson(wiredData, WiredEffectMatchFurni.JsonData.class);
         ...
         for (int i = 0; i < items.length; i++) {
            try {
               ...
               if (stuff.length >= 5) {
                  this.settings
                     .add(
                        new WiredMatchFurniSetting(
                           Integer.parseInt(stuff[0]), stuff[1], Integer.parseInt(stuff[2]), Integer.parseInt(stuff[3]), Integer.parseInt(stuff[4])
                        )
                     );
               }
            } catch (Exception e) {
   ...
   public void serializeWiredData(ServerMessage message, Room room) {
      ...
      TObjectHashIterator var3 = this.settings.iterator();

      while (var3.hasNext()) {
         WiredMatchFurniSetting item = (WiredMatchFurniSetting)var3.next();
   ...
   public boolean saveData(WiredSettings settings, GameClient gameClient) throws WiredSaveException {
      ...
      for (int i = 0; i < itemsCount; i++) {
         ...
         newSettings.add(
            new WiredMatchFurniSetting(
               it.getId(), this.checkForWiredResetPermission && it.allowWiredResetState() ? it.getExtradata() : " ", it.getRotation(), it.getX(), it.getY()
            )
         );
      }
   ...
   private void refresh() {
      ...
      if (room != null && room.isLoaded()) {
         ...
         while (var3.hasNext()) {
            WiredMatchFurniSetting setting = (WiredMatchFurniSetting)var3.next();
            HabboItem item = room.getHabboItem(setting.item_id);
            if (item == null) {
               remove.add(setting);
            }
