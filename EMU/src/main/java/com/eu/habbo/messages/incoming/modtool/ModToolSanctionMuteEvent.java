            ...
            if (Emulator.getConfig().getBoolean("hotel.sanctions.enabled")) {
               ...
               if (modToolSanctionItems != null && !modToolSanctionItemsHashMap.isEmpty()) {
                  ...
                  if (item.probationTimestamp > 0 && item.probationTimestamp >= Emulator.getIntUnixTimestamp()) {
                     ...
                     int muteDurationTimestamp = Math.toIntExact(
                        new Date(System.currentTimeMillis() + modToolSanctionLevelItem.sanctionHourLength * 60 * 60).getTime() / 1000L
                     );
               ...
               this.client
                  .getHabbo()
                  .whisper(Emulator.getTexts().getValue("commands.succes.cmd_mute.muted").replace("%user%", habbo.getHabboInfo().getUsername()));
            }
         } else {
            this.client
               .sendResponse(
                  new ModToolIssueHandledComposer(
