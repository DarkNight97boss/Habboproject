      ...
      if (this.client.getHabbo().hasPermission(Permission.ACC_SUPPORTTOOL)) {
         ...
         if (alertedUser != null) {
            Emulator.getGameEnvironment()
               .getModToolManager()
               .alert(this.client.getHabbo(), alertedUser, this.packet.readString(), SupportUserAlertedReason.CAUTION);
         }
      } else {
         ScripterManager.scripterDetected(
            this.client,
            Emulator.getTexts().getValue("scripter.warning.modtools.kick").replace("%username%", this.client.getHabbo().getHabboInfo().getUsername())
         );
      }
   }
}
