...
public class InteractionSnowboardSlope extends InteractionMultiHeight {
   ...
   public void onPickUp(Room room) {
      TObjectHashIterator var2 = room.getHabbosOnItem(this).iterator();

      while (var2.hasNext()) {
         Habbo habbo = (Habbo)var2.next();
   ...
   public void onMove(Room room, RoomTile oldLocation, RoomTile newLocation) {
      Rectangle newRect = RoomLayout.getRectangle(
         newLocation.x, newLocation.y, this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation()
      );
      ...
      while (var5.hasNext()) {
         ...
         if (habbo.getRoomUnit().getEffectId() == 97
            && !newRect.contains(habbo.getRoomUnit().getCurrentLocation().x, habbo.getRoomUnit().getCurrentLocation().y)) {
            room.giveEffect(habbo, 0, -1);
         }
      }
   }
