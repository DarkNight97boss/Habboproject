...
public class InteractionMultiHeight extends HabboItem {
   ...
   public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
      ...
      if (client == null
         || room.hasRights(client.getHabbo())
         ...
         if (objects.length > 0 && objects[0] instanceof Integer && room != null) {
            ...
            if (this.getBaseItem().getMultiHeights().length > 0) {
               ...
               room.updateTiles(
                  room.getLayout()
                     .getTilesAt(
                        room.getLayout().getTile(this.getX(), this.getY()), this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation()
                     )
               );
   ...
   public void updateUnitsOnItem(Room room) {
      THashSet<RoomTile> occupiedTiles = room.getLayout()
         .getTilesAt(room.getLayout().getTile(this.getX(), this.getY()), this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation());
      TObjectHashIterator var3 = occupiedTiles.iterator();

      while (var3.hasNext()) {
         RoomTile tile = (RoomTile)var3.next();
