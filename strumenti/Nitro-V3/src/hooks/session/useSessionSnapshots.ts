 ...
 */
// Fallback permission grants for stock Arcturus servers that don't emit the
// permission map (UserPermissionsParser optional trailing block from
// Arcturus-Morningstar-Extended ≥ 4.2.10). Backed by securityLevel from
// UserDataSnapshot. Empty map → grant whitelisted keys based on level.
const FALLBACK_PERMISSIONS_BY_LEVEL: ReadonlyArray<{ minLevel: number; keys: ReadonlyArray<string> }> = [
    { minLevel: 2, keys: [ 'acc_supporttool', 'acc_modtool_ticket_q', 'acc_chatlogs', 'acc_user_chatlog' ] },
    { minLevel: 3, keys: [ 'acc_modtools', 'acc_modtool_chatlog', 'acc_modtool_user_alert', 'acc_modtool_user_kick', 'acc_modtool_user_mute', 'acc_modtool_user_ban', 'acc_modtool_user_caution', 'acc_modtool_room_alert', 'acc_modtool_room_kick' ] },
    { minLevel: 4, keys: [ 'acc_anyroomowner', 'acc_anyroomcontroller', 'acc_floorplan_editor', 'acc_unlimited_log', 'acc_camera', 'acc_floorplaneditor', 'acc_room_settings' ] },
    { minLevel: 5, keys: [ 'acc_supertool', 'acc_set_staff', 'acc_command_alert' ] }
];
...
export const useHasPermission = (key: string): boolean =>
{
    ...
    const userData = useUserDataSnapshot();
    const securityLevel = userData?.securityLevel ?? 0;

    return useMemo(() =>
    {
        // Preferred path: server emitted the permission map.
        if(permissions.size > 0) return permissions.get(key) === 1;

        // Fallback for stock Arcturus: securityLevel-based whitelist.
        for(const entry of FALLBACK_PERMISSIONS_BY_LEVEL)
        {
            if(securityLevel >= entry.minLevel && entry.keys.includes(key)) return true;
        }

        return false;
    }, [ permissions, securityLevel, key ]);
};
