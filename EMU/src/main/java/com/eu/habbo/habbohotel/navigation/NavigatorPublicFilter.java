   ...
   public List<SearchResultList> getResult(Habbo habbo) {
      ...
      resultLists.add(
         new SearchResultList(
            ...
            "",
            SearchAction.NONE,
            habbo.getHabboStats().navigatorWindowSettings.getListModeForCategory("official-root", ListMode.THUMBNAILS),
            habbo.getHabboStats().navigatorWindowSettings.getDisplayModeForCategory("official-root"),
            Emulator.getGameEnvironment().getNavigatorManager().getRoomsForCategory("official-root", habbo),
            false,
            showInvisible,
         ...
         if (!category.rooms.isEmpty()) {
            resultLists.add(
               new SearchResultList(
                  ...
                  category.name,
                  SearchAction.NONE,
                  habbo.getHabboStats().navigatorWindowSettings.getListModeForCategory(category.name, category.image),
                  habbo.getHabboStats().navigatorWindowSettings.getDisplayModeForCategory(category.name),
                  category.rooms,
                  true,
                  showInvisible,
                  DisplayOrder.ORDER_NUM,
