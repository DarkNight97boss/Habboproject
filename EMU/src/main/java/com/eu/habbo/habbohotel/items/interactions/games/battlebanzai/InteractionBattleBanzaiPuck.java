...
public class InteractionBattleBanzaiPuck extends InteractionPushable {
   ...
   public int getNextRollDelay(int currentStep, int totalSteps) {
      int t = 2500;
      int var4;
      return totalSteps == 1 ? 500 : 100 * ((var4 = t / t - 1) * var4 * var4 * var4 * var4 + 1) + currentStep * 100;
   }
   ...
   public boolean validMove(Room room, RoomTile from, RoomTile to) {
      ...
      return room.getLayout().tileWalkable(to.x, to.y)
         && (topItem == null || topItem.getBaseItem().allowStack() && !topItem.getBaseItem().allowSit() && !topItem.getBaseItem().allowLay());
   }
