...
public class PetData implements Comparable<PetData> {
   ...
   boolean haveNest(Item nest) {
      return generalNestItems.contains(nest) || this.nestItems.contains(nest);
   }
   ...
   public HabboItem randomNest(THashSet<InteractionNest> items) {
      ...
      while (var3.hasNext()) {
         InteractionNest nest = (InteractionNest)var3.next();
   ...
   boolean haveFoodItem(Item food) {
      return this.foodItems.contains(food) || generalFoodItems.contains(food);
   }
   ...
   public HabboItem randomFoodItem(THashSet<InteractionPetFood> items) {
      ...
      while (var3.hasNext()) {
         InteractionPetFood food = (InteractionPetFood)var3.next();
   ...
   public HabboItem randomDrinkItem(THashSet<InteractionPetDrink> items) {
      ...
      while (var3.hasNext()) {
         InteractionPetDrink drink = (InteractionPetDrink)var3.next();
   ...
   public boolean haveToyItem(Item toy) {
      return this.toyItems.contains(toy) || generalToyItems.contains(toy);
   }

   public HabboItem randomToyItem(THashSet<InteractionPetToy> toys) {
      List<HabboItem> toyList = new ArrayList<>();
      TObjectHashIterator var3 = toys.iterator();

      while (var3.hasNext()) {
         InteractionPetToy toy = (InteractionPetToy)var3.next();
   ...
   public PetVocal randomVocal(PetVocalsType type) {
      ...
      if (this.petVocals.get(type) != null) {
         vocals.addAll((Collection<? extends PetVocal>)this.petVocals.get(type));
      }

      if (generalPetVocals.get(type) != null) {
         vocals.addAll((Collection<? extends PetVocal>)generalPetVocals.get(type));
      }

      return vocals.isEmpty() ? null : vocals.get(Emulator.getRandom().nextInt(vocals.size()));
   }
   ...
   public void reset() {
      ...
      if (generalPetVocals.isEmpty()) {
         for (PetVocalsType type : PetVocalsType.values()) {
            generalPetVocals.put(type, new THashSet());
         }
      }
   }
}
