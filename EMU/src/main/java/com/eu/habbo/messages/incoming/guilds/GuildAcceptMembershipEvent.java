   ...
   public void handle() throws Exception {
      ...
      if (guild != null) {
         ...
         if (userId == this.client.getHabbo().getHabboInfo().getId()
            ...
            || groupMember.getRank().equals(GuildRank.ADMIN)
            || groupMember.getRank().equals(GuildRank.OWNER)
                     ...
                     habbo.getClient()
                        .sendResponse(
                           new GuildInfoComposer(
                              guild, habbo.getClient(), false, Emulator.getGameEnvironment().getGuildManager().getGuildMember(guildId, userId)
                           )
                        );
