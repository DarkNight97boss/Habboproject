   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      if (gameClient.getHabbo().getRoomUnit() != null && gameClient.getHabbo().getRoomUnit().canForcePosture()) {
         ...
         gameClient.getHabbo()
            .getRoomUnit()
            .setBodyRotation(
               RoomUserRotation.values()[gameClient.getHabbo().getRoomUnit().getBodyRotation().getValue()
                  - gameClient.getHabbo().getRoomUnit().getBodyRotation().getValue() % 2]
            );
         ...
         for (int i = 0; i < 3; i++) {
            RoomTile t = gameClient.getHabbo()
               .getHabboInfo()
               .getCurrentRoom()
               .getLayout()
               .getTileInFront(tile, gameClient.getHabbo().getRoomUnit().getBodyRotation().getValue(), i);
