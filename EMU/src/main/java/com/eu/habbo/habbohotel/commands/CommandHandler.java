                     ...
                     if (s.toLowerCase().equals(parts[0].toLowerCase())) {
                        ...
                        if (command.permission == null
                           || gameClient.getHabbo()
                              .hasPermission(
                                 command.permission,
                                 gameClient.getHabbo().getHabboInfo().getCurrentRoom() != null
                                       && gameClient.getHabbo().getHabboInfo().getCurrentRoom().hasRights(gameClient.getHabbo())
                                    || gameClient.getHabbo().hasPermission(Permission.ACC_PLACEFURNI)
                                    || gameClient.getHabbo().getHabboInfo().getCurrentRoom() != null
                                       && gameClient.getHabbo().getHabboInfo().getCurrentRoom().getGuildId() > 0
                                       && gameClient.getHabbo()
                                          .getHabboInfo()
                                          .getCurrentRoom()
                                          .getGuildRightLevel(gameClient.getHabbo())
                  ...
                  if (pet != null && pet.getName().equalsIgnoreCase(args[0])) {
                     ...
                     for (PetCommand command : pet.getPetData().getPetCommands()) {
                        if (command.key.equalsIgnoreCase(s.toString())) {
                           ...
                           } else if (command.level <= pet.getLevel()) {
      ...
      if (Emulator.getGameEnvironment().getPermissionsManager().rankExists(rankId)) {
         ...
         for (Command command : commands.values()) {
            if (!allowedCommands.contains(command)
               && permissions.contains(command.permission)
               && ((Permission)permissions.get(command.permission)).setting != PermissionSetting.DISALLOWED) {
