...
public class GiveRankCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      ...
      if (params.length == 2) {
         gameClient.getHabbo()
            .whisper(
               Emulator.getTexts().getValue("commands.error.cmd_give_rank.missing_rank") + Emulator.getTexts().getValue("commands.description.cmd_give_rank"),
               RoomChatMessageBubbles.ALERT
            );
         return true;
      }
      ...
      if (params.length == 3) {
         ...
         if (rank != null) {
            if (rank.getId() > gameClient.getHabbo().getHabboInfo().getRank().getId()) {
               gameClient.getHabbo()
                  .whisper(
                     Emulator.getTexts().getValue("commands.error.cmd_give_rank.higher").replace("%username%", params[1]).replace("%id%", rank.getName()),
                     RoomChatMessageBubbles.ALERT
                  );
               return true;
            }
            ...
            if (habbo != null) {
               ...
               gameClient.getHabbo()
                  .whisper(
                     Emulator.getTexts().getValue("commands.succes.cmd_give_rank.updated").replace("%id%", rank.getName()).replace("%username%", params[1]),
                     RoomChatMessageBubbles.ALERT
                  );
               return true;
            }
            ...
            gameClient.getHabbo()
               .whisper(
                  Emulator.getTexts().getValue("commands.error.cmd_give_rank.user_offline").replace("%id%", rank.getName()).replace("%username%", params[1]),
                  RoomChatMessageBubbles.ALERT
               );
            return true;
         }
      ...
      gameClient.getHabbo()
         .whisper(
            Emulator.getTexts().getValue("commands.errors.cmd_give_rank.not_found").replace("%id%", params[2]).replace("%username%", params[1]),
            RoomChatMessageBubbles.ALERT
         );
      return true;
   }
