...
public class MessengerBuddy implements Runnable, ISerialize {
   ...
   public void run() {
      try {
         ...
         try {
            PreparedStatement statement = connection.prepareStatement("UPDATE messenger_friendships SET relation = ? WHERE user_one_id = ? AND user_two_id = ?");
   ...
   public void serialize(ServerMessage message) {
      ...
      message.appendInt(this.categoryId);
