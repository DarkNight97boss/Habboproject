...
public class SearchRoomsEvent extends MessageHandler {
   ...
   public void handle() throws Exception {
      ...
      if (cachedResults.containsKey(this.client.getHabbo().getHabboInfo().getRank())) {
         message = (ServerMessage)((THashMap)cachedResults.get(this.client.getHabbo().getHabboInfo().getRank())).get((name + "\t" + query).toLowerCase());
      } else {
      ...
      if (message == null) {
         ...
         THashMap<String, ServerMessage> map = (THashMap<String, ServerMessage>)cachedResults.get(this.client.getHabbo().getHabboInfo().getRank());
         if (map == null) {
            map = new THashMap(1);
         }
