...
public class HabboNavigatorWindowSettings {
         ...
         try {
            ...
            try {
               PreparedStatement statement = connection.prepareStatement(
                  "INSERT INTO users_navigator_settings (user_id, caption, list_type, display) VALUES (?, ?, ?, ?)"
               );
   ...
   public void setDisplayMode(String category, DisplayMode displayMode) {
      HabboNavigatorPersonalDisplayMode personalDisplayMode = (HabboNavigatorPersonalDisplayMode)this.displayModes.get(category);
   ...
   public DisplayMode getDisplayModeForCategory(String category, DisplayMode standard) {
      return this.displayModes.containsKey(category) ? ((HabboNavigatorPersonalDisplayMode)this.displayModes.get(category)).displayMode : standard;
   }
   ...
   public ListMode getListModeForCategory(String category, ListMode standard) {
      return this.displayModes.containsKey(category) ? ((HabboNavigatorPersonalDisplayMode)this.displayModes.get(category)).listMode : standard;
   }

   public void save(Connection connection) {
      try {
         PreparedStatement statement = connection.prepareStatement(
            "UPDATE users_navigator_settings SET list_type = ?, display = ? WHERE user_id = ? AND caption = ? LIMIT 1"
         );

         try {
            for (Entry<String, HabboNavigatorPersonalDisplayMode> set : this.displayModes.entrySet()) {
