...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/threading/runnables/PetEatAction.class */
public class PetEatAction implements Runnable {
    private final Pet pet;
    private final InteractionPetFood food;

    public PetEatAction(Pet pet, InteractionPetFood interactionPetFood) {
        this.pet = pet;
        this.food = interactionPetFood;
    }
    ...
    public void run() {
        ...
        if (this.pet.levelHunger < 20 || this.food == null || Integer.valueOf(this.food.getExtradata()).intValue() >= this.food.getBaseItem().getStateCount()) {
            if (this.food != null && Integer.valueOf(this.food.getExtradata()).intValue() == this.food.getBaseItem().getStateCount()) {
                Emulator.getThreading().run(new QueryDeleteHabboItem(this.food.getId()), 500L);
        ...
        this.food.setExtradata((Integer.valueOf(this.food.getExtradata()).intValue() + 1) + Emulator.PREVIEW);
        ...
        } else if (this.pet.getPetData().getType() == 26) {
