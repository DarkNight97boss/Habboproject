...
public class WiredEffectTriggerStacks extends InteractionWiredEffect {
   ...
   public void serializeWiredData(ServerMessage message, Room room) {
      ...
      invalidTriggers = items.iterator();
      ...
      invalidTriggers = this.items.iterator();
      ...
      if (this.requiresTriggeringUser()) {
         final List<Integer> invalidTriggersx = new ArrayList<>();
         room.getRoomSpecialTypes().getTriggers(this.getX(), this.getY()).forEach(new TObjectProcedure<InteractionWiredTrigger>() {
            public boolean execute(InteractionWiredTrigger object) {
               if (!object.isTriggeredByRoomUnit()) {
                  invalidTriggersx.add(object.getBaseItem().getSpriteId());
               }

               return true;
            }
         });
         message.appendInt(invalidTriggersx.size());

         for (Integer i : invalidTriggersx) {
            message.appendInt(i);
         }
      } else {
         message.appendInt(0);
   ...
   public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
      if (stuff != null && (stuff.length < 1 || !(stuff[stuff.length - 1] instanceof WiredEffectTriggerStacks))) {
         ...
         TObjectHashIterator newStuff = this.items.iterator();

         while (newStuff.hasNext()) {
            HabboItem item = (HabboItem)newStuff.next();
            boolean found = false;
            TObjectHashIterator var8 = usedTiles.iterator();

            while (var8.hasNext()) {
               RoomTile tile = (RoomTile)var8.next();
         ...
         Object[] newStuffx = new Object[stuff.length + 1];
         System.arraycopy(stuff, 0, newStuffx, 0, stuff.length);
         newStuffx[newStuffx.length - 1] = this;
   ...
   public String getWiredData() {
      return WiredHandler.getGsonBuilder()
         .create()
         .toJson(new WiredEffectTriggerStacks.JsonData(this.getDelay(), this.items.stream().map(HabboItem::getId).collect(Collectors.toList())));
   }
   ...
   public void loadWiredData(ResultSet set, Room room) throws SQLException {
      ...
      if (wiredData.startsWith("{")) {
         WiredEffectTriggerStacks.JsonData data = (WiredEffectTriggerStacks.JsonData)WiredHandler.getGsonBuilder()
            .create()
            .fromJson(wiredData, WiredEffectTriggerStacks.JsonData.class);
