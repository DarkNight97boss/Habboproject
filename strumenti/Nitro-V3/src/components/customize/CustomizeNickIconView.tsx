...
import { INickIconItem, IPrefixItem, LocalizeText, PRESET_PREFIX_EFFECTS, PRESET_PREFIX_FONTS, SendMessageComposer, getPrefixEffectStyle, getPrefixFontStyle, parsePrefixColors } from '../../api';
...
export const CustomizeNickIconView: FC<{}> = () =>
{
    ...
    return (
        <NitroCardView className="customize-nick-icon-window w-[680px] max-w-[95vw]" theme="primary-slim" uniqueKey="customize-nick-icons">
            <NitroCardHeaderView headerText={ LocalizeText('customize.nick_icon.title') } onCloseClick={ () => setIsVisible(false) } />
            <NitroCardTabsView>
                <NitroCardTabsItemView isActive={ activeTab === 'icons' } onClick={ () => setActiveTab('icons') }>
                    <Text>{ LocalizeText('customize.nick_icon.tabs.icons') }</Text>
                </NitroCardTabsItemView>
                <NitroCardTabsItemView isActive={ activeTab === 'prefix' } onClick={ () => setActiveTab('prefix') }>
                    <Text>{ LocalizeText('customize.nick_icon.tabs.prefix') }</Text>
                </NitroCardTabsItemView>
                <NitroCardTabsItemView isActive={ activeTab === 'settings' } onClick={ () => setActiveTab('settings') }>
                    <Text>{ LocalizeText('customize.nick_icon.tabs.settings') }</Text>
                </NitroCardTabsItemView>
            </NitroCardTabsView>
            <NitroCardContentView className="flex max-h-[78vh] flex-col gap-3 overflow-y-auto text-black">
                <div className="rounded border border-black/10 bg-black/5 p-3">
                    <Text bold>{ LocalizeText('customize.nick_icon.live_preview') }</Text>
                    <div className="mt-2 flex min-h-[54px] items-center justify-center rounded border border-black/10 bg-[#1f2937] px-3 py-2 text-white">
                        <UserIdentityView
                            ...
                            username={ LocalizeText('customize.nick_icon.preview.username') } />
                    </div>
                </div>
                ...
                { activeTab === 'icons' &&
                    <>
                        <div className="rounded border border-black/10 bg-black/5 p-2 text-[11px] leading-4">
                            { LocalizeText('customize.nick_icon.icons.help') }
                        </div>
                            ...
                            { iconItems.map(item =>
                            {
                                ...
                                return (
                                    <div
                                        ...
                                        { item.active && <span className="absolute right-1 top-1 rounded bg-[#1e7295] px-1.5 py-0.5 text-[9px] font-bold uppercase text-white">{ LocalizeText('customize.nick_icon.badge.active') }</span> }
                                        ...
                                        <div className="flex flex-col items-center gap-1 text-center text-[11px]">
                                            <span>{ item.owned ? (item.active ? LocalizeText('customize.nick_icon.status.owned_active') : LocalizeText('customize.nick_icon.status.owned')) : LocalizeText('customize.nick_icon.status.locked') }</span>
                                        ...
                                        <Button disabled={ isLoading } onClick={ () => handleIconAction(item) }>
                                            { !item.owned && LocalizeText('customize.nick_icon.action.buy') }
                                            { item.owned && !item.active && LocalizeText('customize.nick_icon.action.activate') }
                                            { item.owned && item.active && LocalizeText('customize.nick_icon.action.deactivate') }
                                        </Button>
                                    </div>
                                );
                            }) }
                ...
                { activeTab === 'prefix' &&
                    <div className="flex flex-col gap-3">
                        <div className="rounded border border-black/10 bg-black/5 p-1">
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    { LocalizeText('customize.nick_icon.prefix.tab.library') }
                                </button>
                                <button
                                    ...
                                    { LocalizeText('customize.nick_icon.prefix.tab.custom') }
                                </button>
                            </div>
                        </div>
                        ...
                        { activePrefixSubTab === 'library' &&
                            <>
                                <div className="rounded border border-black/10 bg-black/5 p-2 text-[11px] leading-4">
                                    { LocalizeText('customize.nick_icon.prefix.help') }
                                </div>
                                    ...
                                    { combinedPrefixes.map(item => (
                                        <div key={ `${ item.catalogPrefixId || 'custom' }-${ item.ownedPrefixId || item.id }` } className={ `relative flex min-h-[96px] flex-col gap-2 rounded border p-2.5 ${ item.active ? 'border-[#1e7295] bg-[#dff3fb]' : 'border-black/10 bg-black/5' }` }>
                                            ...
                                            <div className="flex flex-col gap-1 text-[11px]">
                                                <span>{ item.owned ? (item.active ? LocalizeText('customize.nick_icon.status.owned_active') : LocalizeText('customize.nick_icon.status.owned')) : LocalizeText('customize.nick_icon.status.locked') }</span>
                                                <span className="truncate">{ item.displayName || item.text }{ item.isCustom ? ` - ${ LocalizeText('customize.nick_icon.prefix.tag.custom') }` : '' }</span>
                                                <span className="inline-flex items-center gap-1">
                                                    <LayoutCurrencyIcon type={ item.pointsType } />
                                                    { item.points }
                                                </span>
                        ...
                        <div className="rounded border border-black/10 bg-black/5 p-3">
                            <div className="mb-2 flex items-center justify-between">
                                <Text bold>{ LocalizeText('customize.nick_icon.custom_prefix.title') }</Text>
                                <Button disabled={ isLoading } onClick={ refreshCustomizeData }>{ LocalizeText('customize.nick_icon.action.refresh') }</Button>
                            </div>
                            <div className="mt-2 flex flex-col gap-2">
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        placeholder={ LocalizeText('customize.nick_icon.custom_prefix.placeholder') }
                                        type="text"
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    <button className="rounded border border-black/10 bg-white px-3 py-2 text-sm" type="button" onClick={ () => setShowEmojiPicker(true) }>
                                        { customPrefixIcon || LocalizeText('customize.nick_icon.custom_prefix.emoji') }
                                    </button>
                                    { !!customPrefixIcon && <Button onClick={ () => setCustomPrefixIcon('') }>{ LocalizeText('customize.nick_icon.action.clear') }</Button> }
                                </div>
                                <div className="rounded border border-black/10 bg-white p-2">
                                    <div className="mb-2 text-[11px] leading-4 text-black/70">
                                        { LocalizeText('customize.nick_icon.custom_prefix.colors.help') }
                                    </div>
                                ...
                                <div className="rounded border border-black/10 bg-white p-2">
                                    <div className="mb-2 text-[11px] leading-4 text-black/70">
                                        { LocalizeText('customize.nick_icon.custom_prefix.effect') }
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <select
                                            ...
                                            <optgroup label={ LocalizeText('customize.nick_icon.tier.basic') }>
                                            ...
                                            <optgroup label={ LocalizeText('customize.nick_icon.tier.premium') }>
                                ...
                                <div className="rounded border border-black/10 bg-white p-2">
                                    <div className="mb-2 text-[11px] leading-4 text-black/70">
                                        { LocalizeText('customize.nick_icon.custom_prefix.font') }
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <select
                                            className="flex-1 rounded border border-black/10 bg-white px-2 py-2 text-sm"
                                    ...
                                    { !!customPrefixFont &&
                                        <div className="mt-2 text-[10px] leading-4 text-black/60">
                                            { LocalizeText('customize.nick_icon.custom_prefix.font.premium.note') }
                                        </div> }
                                </div>
                                <div className="rounded border border-black/10 bg-[#1f2937] px-3 py-2 text-white" style={ customPrefixPreviewStyle }>
                                    <UserIdentityView
                                        ...
                                        prefixText={ customPrefixText || LocalizeText('customize.nick_icon.custom_prefix.preview.fallback') }
                                        username={ LocalizeText('customize.nick_icon.preview.username') } />
                                </div>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-[12px]">
                                        { customPrefixTotalCredits > 0 && <span>{ customPrefixTotalCredits } { LocalizeText('customize.nick_icon.currency.credits') }</span> }
                                    ...
                                    <Button disabled={ !customPrefixIsValid || isLoading } onClick={ handleCustomPrefixPurchase }>
                                        { LocalizeText('customize.nick_icon.custom_prefix.buy') }
                                    </Button>
                                </div>
                            </div>
                        </div> }
                ...
                { activeTab === 'settings' &&
                    <div className="flex flex-col gap-3">
                        <div className="rounded border border-black/10 bg-black/5 p-3">
                            <Text bold>{ LocalizeText('customize.nick_icon.display_order') }</Text>
                        ...
                        <div className="rounded border border-black/10 bg-black/5 p-3">
                            <div className="mb-2 flex items-center justify-between">
                                <Text bold>{ LocalizeText('customize.nick_icon.refresh_data') }</Text>
                                <Button disabled={ isLoading } onClick={ refreshCustomizeData }>{ LocalizeText('customize.nick_icon.action.refresh') }</Button>
                            </div>
                            <div className="text-[11px] leading-4 text-black/70">
                                { LocalizeText('customize.nick_icon.settings.help') }
                            </div>
                        </div>
                    </div> }
