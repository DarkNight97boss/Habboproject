...
public class CatalogPurchaseLogEntry implements Runnable, DatabaseLoggable {
   ...
   public CatalogPurchaseLogEntry(
      int timestamp, int userId, int catalogItemId, String itemIds, String catalogName, int costCredits, int costPoints, int pointsType, int amount
   ) {
   ...
   public String getQuery() {
      return "INSERT INTO `logs_shop_purchases` (timestamp, user_id, catalog_item_id, item_ids, catalog_name, cost_credits, cost_points, points_type, amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
   }
