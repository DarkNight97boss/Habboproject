    ...
    const submitReport = () =>
    {
        ...
        switch(activeReport.reportType)
        {
            ...
            case ReportType.ROOM: {
                // reportedChats can be empty (general support ticket — no specific user/chat),
                // in that case fall back to roomId=0 (server handler accepts it).
                const reportedRoomId = (activeReport.roomId > 0)
                    ? activeReport.roomId
                    : (activeReport.reportedChats.length > 0 ? activeReport.reportedChats[0].roomId : 0);
