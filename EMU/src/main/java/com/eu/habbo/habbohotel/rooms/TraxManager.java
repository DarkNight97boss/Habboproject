...
public class TraxManager implements Disposable {
   ...
   public TraxManager(Room room) {
      ...
      if (this.jukeBox == null) {
         TObjectHashIterator var2 = room.getRoomSpecialTypes().getItemsOfType(InteractionJukeBox.class).iterator();
         ...
         label129: {
            label139: {
               try {
                  ...
                  label131: {
                     label132: {
                        statement = connection.prepareStatement("SELECT * FROM room_trax WHERE room_id = ?");
                                    ...
                                    try {
                                       if (set.next()) {
                                          ...
                                          if (jukebox != null) {
                                             ...
                                             var5 = (InteractionJukeBox)jukebox;
                                             break label110;
                                          }
                                       }
   ...
   public void loadPlaylist() {
      if (this.jukeBox != null) {
         ...
         try {
            ...
            try {
               PreparedStatement statement = connection.prepareStatement("SELECT * FROM trax_playlist WHERE trax_item_id = ?");
   ...
   public static void deleteSongFromPlaylist(int jukebox_id, int song_id) {
      try {
         ...
         try {
            PreparedStatement statement = connection.prepareStatement("DELETE FROM trax_playlist WHERE trax_item_id = ? AND item_id = ? LIMIT 1");
   ...
   public void addTraxOnRoom(InteractionJukeBox jukeBox) {
      if (this.jukeBox == null) {
         try {
            ...
            try {
               PreparedStatement statement_1 = connection.prepareStatement("INSERT INTO room_trax (room_id, trax_item_id) VALUES (?, ?)");
   ...
   public void removeTraxOnRoom(InteractionJukeBox jukeBox) {
      if (this.jukeBox.getId() == jukeBox.getId()) {
         try {
            Connection connection = Emulator.getDatabase().getDataSource().getConnection();

            try {
               PreparedStatement statement_1 = connection.prepareStatement("DELETE FROM room_trax WHERE room_id = ?");
   ...
   public void play(int index, Habbo starter) {
      ...
      if (!this.songs.isEmpty()) {
         ...
         this.room
            .sendComposer(
               new JukeBoxNowPlayingMessageComposer(
                     Emulator.getGameEnvironment().getItemManager().getSoundTrack(this.currentlyPlaying.getSongId()), this.playingIndex, 0
                  )
                  .compose()
            );
      } else {
   ...
   public void stop() {
      if (this.starter != null && this.cycleStartedTimestamp > 0) {
         AchievementManager.progressAchievement(
            ...
            (Emulator.getIntUnixTimestamp() - this.cycleStartedTimestamp) / 60
         );
      }
   ...
   public SoundTrack currentSong() {
      return !this.songs.isEmpty() && this.playingIndex < this.songs.size()
         ? Emulator.getGameEnvironment().getItemManager().getSoundTrack(this.songs.get(this.playingIndex).getSongId())
         : null;
   }
         ...
         if (this.songsLimit < this.songs.size() + 1) {
            ...
            if (track != null) {
               ...
               try {
                  ...
                  try {
                     PreparedStatement statement = connection.prepareStatement("INSERT INTO trax_playlist (trax_item_id, item_id) VALUES (?, ?)");
   ...
   public void removeSong(int itemId) {
      if (!this.songs.isEmpty()) {
         ...
         if (musicDisc != null) {
            ...
            this.totalLength = this.totalLength - Emulator.getGameEnvironment().getItemManager().getSoundTrack(musicDisc.getSongId()).getLength();
            ...
            try {
               ...
               try {
                  while (set.next()) {
                     ...
                     if (musicDisc != null && musicDisc instanceof InteractionMusicDisc && musicDisc.getRoomId() == -1) {
                        musicDisc.setRoomId(0);
                        musicDisc.needsUpdate(true);
                        Emulator.getThreading().run(musicDisc);
                        Habbo owner = Emulator.getGameEnvironment().getHabboManager().getHabbo(musicDisc.getUserId());
   ...
   public int getSongsLimit(InteractionJukeBox jukeBox) {
      return "jukebox_big".equals(jukeBox.getBaseItem().getName()) ? LARGE_JUKEBOX_LIMIT : NORMAL_JUKEBOX_LIMIT;
   }
   ...
   public void updateCurrentPlayingSong(Habbo habbo) {
      if (this.isPlaying()) {
         habbo.getClient()
            .sendResponse(
               new JukeBoxNowPlayingMessageComposer(
                  Emulator.getGameEnvironment().getItemManager().getSoundTrack(this.currentlyPlaying.getSongId()),
                  this.playingIndex,
