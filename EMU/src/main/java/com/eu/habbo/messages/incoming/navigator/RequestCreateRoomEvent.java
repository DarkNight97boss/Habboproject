         ...
         if (category == null || category.getMinRank() > this.client.getHabbo().getHabboInfo().getRank().getId()) {
            ...
            if (tradeType <= 2) {
               if (name.trim().length() >= 3
                  && name.length() <= 25
                  && Emulator.getGameEnvironment().getWordFilter().filter(name, this.client.getHabbo()).equals(name)) {
                  if (description.length() <= 128
                     && Emulator.getGameEnvironment().getWordFilter().filter(description, this.client.getHabbo()).equals(description)) {
                     ...
                     if (count >= max) {
                        ...
                        Room room = Emulator.getGameEnvironment()
                           .getRoomManager()
                           .createRoomForHabbo(this.client.getHabbo(), name, description, modelName, maxUsers, categoryId, tradeType);
                        if (room != null) {
