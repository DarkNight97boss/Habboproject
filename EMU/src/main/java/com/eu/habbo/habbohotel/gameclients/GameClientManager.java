...
public class GameClientManager {
   ...
   private void disposeClient(Channel channel) {
      GameClient client = (GameClient)channel.attr(GameServerAttributes.CLIENT).get();
      if (client != null) {
         client.dispose();
      }
   ...
   public boolean containsHabbo(Integer id) {
      if (!this.clients.isEmpty()) {
         for (GameClient client : this.clients.values()) {
            if (client.getHabbo() != null && client.getHabbo().getHabboInfo() != null && client.getHabbo().getHabboInfo().getId() == id) {
               return true;
            }
         }
      }
   ...
   public Habbo getHabbo(int id) {
      for (GameClient client : this.clients.values()) {
         if (client.getHabbo() != null && client.getHabbo().getHabboInfo().getId() == id) {
   ...
   public Habbo getHabbo(String username) {
      for (GameClient client : this.clients.values()) {
         if (client.getHabbo() != null && client.getHabbo().getHabboInfo().getUsername().equalsIgnoreCase(username)) {
            return client.getHabbo();
         }
      }
   ...
   public List<Habbo> getHabbosWithIP(String ip) {
      ...
      for (GameClient client : this.clients.values()) {
         if (client.getHabbo() != null && client.getHabbo().getHabboInfo() != null && client.getHabbo().getHabboInfo().getIpLogin().equalsIgnoreCase(ip)) {
   ...
   public void sendBroadcastResponse(ServerMessage message, String minPermission, GameClient exclude) {
      for (GameClient client : this.clients.values()) {
         if (!client.equals(exclude) && client.getHabbo() != null && client.getHabbo().hasPermission(minPermission)) {
            client.sendResponse(message);
         }
      }
   }
