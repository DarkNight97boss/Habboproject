...
public class DisconnectCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      if (params.length < 2) {
         ...
         return true;
      } else if (params[1].toLowerCase().equals(gameClient.getHabbo().getHabboInfo().getUsername().toLowerCase())) {
         ...
         if (target == null) {
            ...
            return true;
         } else if (target.getHabboInfo().getRank().getId() > gameClient.getHabbo().getHabboInfo().getRank().getId()) {
            ...
            gameClient.getHabbo()
               .whisper(Emulator.getTexts().getValue("commands.succes.cmd_disconnect.disconnected").replace("%user%", params[1]), RoomChatMessageBubbles.ALERT);
            return true;
         }
      }
   }
