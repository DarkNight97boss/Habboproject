         ...
         if (room.getOwnerId() == this.client.getHabbo().getHabboInfo().getId()
            ...
            if (item.getBaseItem().getName().equals("floor")) {
               ...
               AchievementManager.progressAchievement(
                  this.client.getHabbo(), Emulator.getGameEnvironment().getAchievementManager().getAchievement("RoomDecoFloor")
               );
            } else if (item.getBaseItem().getName().equals("wallpaper")) {
               room.setWallPaint(item.getExtradata());
               AchievementManager.progressAchievement(
                  this.client.getHabbo(), Emulator.getGameEnvironment().getAchievementManager().getAchievement("RoomDecoWallpaper")
               );
            } else {
               if (!item.getBaseItem().getName().equals("landscape")) {
                  return;
               }

               room.setBackgroundPaint(item.getExtradata());
               AchievementManager.progressAchievement(
                  this.client.getHabbo(), Emulator.getGameEnvironment().getAchievementManager().getAchievement("RoomDecoLandscape")
               );
            }
