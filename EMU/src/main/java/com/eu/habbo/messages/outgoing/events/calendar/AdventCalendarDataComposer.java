...
public class AdventCalendarDataComposer extends MessageComposer {
   ...
   public AdventCalendarDataComposer(
      String eventName, String campaignImage, int totalDays, int currentDay, ArrayList<CalendarRewardClaimed> unlocked, boolean lockExpired
   ) {
   ...
   protected ServerMessage composeInternal() {
      ...
      if (this.currentDay > 1) {
         expired.remove(this.currentDay - 2);
      }

      if (this.currentDay > 0) {
         expired.remove(this.currentDay - 1);
      }

      this.unlocked.forEach(claimed -> {
         this.response.appendInt(claimed.getDay());
