...
public class CrackableExplode implements Runnable {
   ...
   public void run() {
      if (this.habboItem.getRoomId() != 0) {
         ...
         if (rewardItem != null) {
            HabboItem newItem = Emulator.getGameEnvironment()
               .getItemManager()
               .createItem(this.habboItem.allowAnyone() ? this.habbo.getHabboInfo().getId() : this.habboItem.getUserId(), rewardItem, 0, 0, "");
            if (newItem != null) {
               if (!this.toInventory && newItem.getBaseItem().getType() != FurnitureType.WALL) {
                  ...
                  this.room.sendComposer(new AddFloorItemComposer(newItem, (String)this.room.getFurniOwnerNames().get(newItem.getUserId())).compose());
               } else {
