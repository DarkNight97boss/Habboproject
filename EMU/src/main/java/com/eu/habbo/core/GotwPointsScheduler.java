...
public class GotwPointsScheduler extends Scheduler {
   ...
   public void run() {
      ...
      for (Entry<Integer, Habbo> map : Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos().entrySet()) {
         ...
         try {
            if (habbo != null && (habbo.getHabboInfo().getCurrentRoom() != null || !IGNORE_HOTEL_VIEW) && (!habbo.getRoomUnit().isIdle() || !IGNORE_IDLED)) {
               boolean found = false;
               ...
               if (found || type != -1) {
                  habbo.givePoints(
                     type, (int)(habbo.getHabboInfo().getRank().getGotwTimerAmount() * (habbo.getHabboStats().hasActiveClub() ? HC_MODIFIER : 1.0))
                  );
               }
            }
         } catch (Exception e) {
