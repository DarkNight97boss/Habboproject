...
public class VisitorBot extends Bot {
   ...
   public void onUserSay(RoomChatMessage message) {
      if (!this.showedLog && message.getMessage().equalsIgnoreCase(Emulator.getTexts().getValue("generic.yes"))) {
         this.showedLog = true;
         String visitMessage = Emulator.getTexts().getValue("bots.visitor.list").replace("%count%", this.visits.size() + "");
         StringBuilder list = new StringBuilder();
         TObjectHashIterator var4 = this.visits.iterator();

         while (var4.hasNext()) {
            ModToolRoomVisit visit = (ModToolRoomVisit)var4.next();
            list.append("\r");
