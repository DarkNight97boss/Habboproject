...
public class UnmuteCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      ...
      if (habbo == null) {
         gameClient.getHabbo()
            .whisper(Emulator.getTexts().getValue("commands.error.cmd_unmute.not_found").replace("%user%", params[1]), RoomChatMessageBubbles.ALERT);
         return true;
      }
      ...
      if (!habbo.getHabboStats().allowTalk() || habbo.getHabboInfo().getCurrentRoom() != null && habbo.getHabboInfo().getCurrentRoom().isMuted(habbo)) {
         ...
         gameClient.getHabbo()
            .whisper(Emulator.getTexts().getValue("commands.error.cmd_unmute.not_muted").replace("%user%", params[1]), RoomChatMessageBubbles.ALERT);
         return true;
      }
   }
}
