...
public class CatalogItem implements ISerialize, Runnable, Comparable<CatalogItem> {
   ...
   public int getLimitedSells() {
      ...
      return ltdConfig != null ? this.limitedStack - ltdConfig.available() : this.limitedStack;
   }
            ...
            if (!itemId.isEmpty()) {
               ...
               try {
                  ...
                  LOGGER.info(
                     "Invalid value ("
                        + itemId
                        + ") for items_base column for catalog_item id ("
                        + this.id
                        + "). Value must be integer or of the format of integer:amount;integer:amount"
                  );
                  continue;
               }
   ...
   public int getItemAmount(int id) {
      return this.bundle.containsKey(id) ? this.bundle.get(id) : this.amount;
   }
   ...
   public void run() {
      if (this.needsUpdate) {
         try {
            ...
            try {
               PreparedStatement statement = connection.prepareStatement("UPDATE catalog_items SET limited_sells = ?, page_id = ? WHERE id = ?");
   ...
   public int compareTo(CatalogItem catalogItem) {
      return CatalogManager.SORT_USING_ORDERNUM ? this.getOrderNumber() - catalogItem.getOrderNumber() : this.getId() - catalogItem.getId();
   }
}
