      ...
      if (habbo != null) {
         if (habbo == gameClient.getHabbo().getHabboInfo()) {
            ...
            int count = Emulator.getGameEnvironment()
               .getModToolManager()
               .ban(habbo.getId(), gameClient.getHabbo(), reason.toString(), 315569260, ModToolBanType.MACHINE, -1)
               .size();
            gameClient.getHabbo()
               .whisper(Emulator.getTexts().getValue("commands.succes.cmd_machine_ban").replace("%count%", count + ""), RoomChatMessageBubbles.ALERT);
            return true;
         }
      } else {
