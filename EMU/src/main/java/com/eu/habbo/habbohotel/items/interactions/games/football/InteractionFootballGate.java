...
public class InteractionFootballGate extends HabboItem {
   ...
   private static void removeLook(Habbo habbo) {
      if (habbo.getHabboStats().cache.containsKey("fball_gate_look")) {
         habbo.getHabboInfo().setLook((String)habbo.getHabboStats().cache.get("fball_gate_look"));
         habbo.getHabboStats().cache.remove("fball_gate_look");
   ...
   public void onWalkOn(RoomUnit roomUnit, Room room, Object[] objects) throws Exception {
      ...
      if (habbo != null) {
         if (habbo.getHabboStats().cache.containsKey("fball_gate_look")) {
            String oldlook = (String)habbo.getHabboStats().cache.get("fball_gate_look");
               ...
               habbo.getHabboInfo()
                  .setLook(
                     ClothingValidationManager.VALIDATE_ON_FBALLGATE
                        ? ClothingValidationManager.validateLook(habbo, lookEvent.newLook, lookEvent.gender.name())
                        : lookEvent.newLook
                  );
            ...
            Emulator.getPluginManager().fireEvent(lookEvent);
            if (!lookEvent.isCancelled()) {
               habbo.getHabboStats().cache.put("fball_gate_look", habbo.getHabboInfo().getLook());
               habbo.getHabboInfo()
                  .setLook(
                     ClothingValidationManager.VALIDATE_ON_FBALLGATE
                        ? ClothingValidationManager.validateLook(habbo, lookEvent.newLook, lookEvent.gender.name())
