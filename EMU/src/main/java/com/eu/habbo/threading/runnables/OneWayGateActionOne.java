...
public class OneWayGateActionOne implements Runnable {
   ...
   public void run() {
      ...
      RoomTile t = this.room
         .getLayout()
         .getTileInFront(this.room.getLayout().getTile(this.oneWayGate.getX(), this.oneWayGate.getY()), (this.oneWayGate.getRotation() + 4) % 8);
      ...
      if (t.isWalkable()) {
         if (this.room.tileWalkable(t)
            && this.client.getHabbo().getRoomUnit().getX() == this.oneWayGate.getX()
            && this.client.getHabbo().getRoomUnit().getY() == this.oneWayGate.getY()) {
         ...
         } else if (!this.oneWayGate.getExtradata().equals("0")) {
            Emulator.getThreading().run(new HabboItemNewState(this.oneWayGate, this.room, "0"), 1000L);
         }
      }
   }
