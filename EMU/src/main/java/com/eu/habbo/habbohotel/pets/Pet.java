...
public class Pet implements ISerialize, Runnable {
   ...
   public String bornDate() {
      ...
      return cal.get(5) + "/" + cal.get(2) + "/" + cal.get(1);
   }
            ...
            try {
               if (this.id > 0) {
                  PreparedStatement statement = connection.prepareStatement(
                     "UPDATE users_pets SET room_id = ?, experience = ?, energy = ?, respect = ?, x = ?, y = ?, z = ?, rot = ?, hunger = ?, thirst = ?, happyness = ?, created = ? WHERE id = ?"
                  );
   ...
   public void clearPosture() {
      ...
      if (this.task == null) {
         ...
         for (Entry<RoomUnitStatus, String> entry : keys.entrySet()) {
   ...
   protected void levelUp() {
      if (this.level < PetManager.experiences.length + 1) {
         ...
         AchievementManager.progressAchievement(
            ...
            Emulator.getGameEnvironment().getAchievementManager().getAchievement("PetLevelUp")
         );
