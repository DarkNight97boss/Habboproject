...
public class RoomUnitWalkToLocation implements Runnable {
   ...
   public RoomUnitWalkToLocation(RoomUnit walker, RoomTile goalTile, Room room, Runnable targetReached, Runnable failedReached) {
      ...
      if (targetReached != null) {
         this.targetReached.add(targetReached);
      }

      this.failedReached = new ArrayList<>();
      if (failedReached != null) {
         this.targetReached.add(failedReached);
      }
   }
   ...
   public void run() {
      if (this.goalTile == null
         || this.walker == null
         || this.room == null
         || this.walker.getRoom() == null
         || this.walker.getRoom().getId() != this.room.getId()) {
         this.onFail();
      } else if (this.walker.getCurrentLocation().equals(this.goalTile)) {
         this.onSuccess();
