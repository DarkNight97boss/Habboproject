...
public class InteractionPetBreedingNest extends HabboItem {
   ...
   public void onWalkOn(RoomUnit roomUnit, Room room, Object[] objects) throws Exception {
      ...
      if (pet != null && !this.boxFull()) {
         ...
         if (this.boxFull()) {
            ...
            if (ownerPetOne != null
               && ownerPetTwo != null
               && this.petOne.getPetData().getType() == this.petTwo.getPetData().getType()
               && this.petOne.getPetData().getOffspringType() != -1) {
               ownerPetTwo.getClient()
                  .sendResponse(
                     new PetBreedingResultComposer(
                        this.getId(),
   ...
   public void stopBreeding(Habbo habbo) {
      ...
      if (this.petOne != null) {
         habbo.getClient().sendResponse(new PetPackageNameValidationComposer(this.getId(), 0, ""));
      }

      if (this.petTwo.getUserId() != habbo.getHabboInfo().getId()) {
         Habbo owner = this.petTwo.getRoom().getHabbo(this.petTwo.getUserId());
         if (owner != null) {
            owner.getClient().sendResponse(new PetPackageNameValidationComposer(this.getId(), 0, ""));
         }
      }
            ...
            () -> {
               Pet offspring = Emulator.getGameEnvironment()
                  ...
                  .createPet(
                     ...
                     (int)Math.min(Math.round(Math.max(1.0, PetManager.getNormalDistributionForBreeding(petOne.getLevel(), petTwo.getLevel()).sample())), 20L),
                     name,
                     habbo.getClient()
                  );
               ...
               habbo.getClient()
                  .sendResponse(new PetBreedingCompleted(offspring.getId(), Emulator.getGameEnvironment().getPetManager().getRarityForOffspring(offspring)));
