...
{
    const { openIssues = null, handleIssue = null, setCurrentTab = null } = props;
    ...
    const pickIssue = (issueId: number) =>
    {
        ...
        // UX upgrade ported from nitro-react: immediately switch to "My Issues"
        // tab and open the detail panel. The IssueInfoMessageEvent from the server
        // arrives ~1 RTT later and updates the ticket state in the local cache.
        if(setCurrentTab) setCurrentTab(1);
        if(handleIssue) handleIssue(issueId);
