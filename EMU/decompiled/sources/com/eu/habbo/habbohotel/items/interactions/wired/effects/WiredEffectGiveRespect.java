...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/effects/WiredEffectGiveRespect.class */
public class WiredEffectGiveRespect extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;
    private int respects;

    /* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/effects/WiredEffectGiveRespect$JsonData.class */
    ...
    public WiredEffectGiveRespect(ResultSet resultSet, Item item) throws SQLException {
        super(resultSet, item);
        this.respects = 0;
    }

    public WiredEffectGiveRespect(int i, int i2, Item item, String str, int i3, int i4) {
    ...
    public void serializeWiredData(ServerMessage serverMessage, Room room) {
        ...
        serverMessage.appendString(this.respects + Emulator.PREVIEW);
        serverMessage.appendInt((Integer) 0);
        serverMessage.appendInt((Integer) 0);
        ...
        room.getRoomSpecialTypes().getTriggers(getX(), getY()).forEach(new TObjectProcedure<InteractionWiredTrigger>() { // from class: com.eu.habbo.habbohotel.items.interactions.wired.effects.WiredEffectGiveRespect.1
    ...
    public boolean saveData(WiredSettings wiredSettings, GameClient gameClient) {
        try {
            this.respects = Integer.parseInt(wiredSettings.getStringParam());
    ...
    public String getWiredData() {
        return WiredHandler.getGsonBuilder().create().toJson(new JsonData(this.respects, getDelay()));
    }
    ...
    public void loadWiredData(ResultSet resultSet, Room room) throws SQLException {
        ...
        if (string.startsWith("{")) {
            ...
            this.respects = jsonData.amount;
            setDelay(jsonData.delay);
        } else {
            String[] strArrSplit = string.split("\t");
            this.respects = 0;
            if (strArrSplit.length >= 2) {
                super.setDelay(Integer.valueOf(strArrSplit[0]).intValue());
                try {
                    this.respects = Integer.valueOf(strArrSplit[1]).intValue();
                } catch (Exception e) {
                }
            }
            needsUpdate(true);
