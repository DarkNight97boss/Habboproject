...
public class AdventCalendarProductComposer extends MessageComposer {
   ...
   protected ServerMessage composeInternal() {
      ...
      String className = "";
      String productName = this.rewardObject
         .getProductName()
         .replace("%credits%", String.valueOf(this.rewardObject.getCredits()))
         .replace(
            "%pixels%", String.valueOf((int)(this.rewardObject.getPixels() * (this.habbo.getHabboStats().hasActiveClub() ? CalendarManager.HC_MODIFIER : 1.0)))
         )
      ...
      if (this.rewardObject.getSubscriptionType() != null) {
         productName = productName.replace("%subscription_type%", this.rewardObject.getSubscriptionType())
            .replace("%subscription_days%", String.valueOf(this.rewardObject.getSubscriptionDays()));
      }
