...
<head>
	...
	<title>ZabboASE &bull; View OOTW</title>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		<div class="row">
			<ol class="breadcrumb">
				...
				<li>Dashboard</li> <li>Management</li> <li class="active">View OOTW</li>
			</ol>
		</div><!--/.row-->
		
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">View OOTW</h1>
			</div>
		</div><!--/.row-->
...
while ($n = mysql_fetch_assoc($getNews)) {
	$username = $n['user'];
$grabUser = mysql_query("SELECT * FROM users WHERE id = '" . $id . "' LIMIT 1");
...
echo '<tr>
	<td><span class="badge" style="width: 50px;height: 115px;"><img style="margin-left: -15px;margin-top: -7px;" src="{imager}/?figure=' . $n['look'] . '&amp;size=m&amp;direction=1&amp;head_direction=1&amp;gesture=sml"><img style="margin-left: -15px;margin-top: -7px;" src="{imager}' . $n['look'] . '&amp;size=m&amp;direction=3&amp;head_direction=3&amp;gesture=sml"><br/><img style="margin-left: -15px;margin-top: -7px;" src="{imager}' . $n['look'] . '&amp;size=m&amp;direction=5&amp;head_direction=5&amp;gesture=sml"><img style="margin-left: -15px;margin-top: -7px;" src="{imager}' . $n['look'] . '&amp;size=m&amp;direction=7&amp;head_direction=7&amp;gesture=sml"></span></td>
...
?>
										...
										<?php
$id = intval($_GET['id']);
$existq = mysql_query("SELECT * FROM users_submissions WHERE id = '$id'") or die("SQL ERROR at ".__FILE__.":".__LINE__." => ".mysql_error());
...
mysql_query("UPDATE users_submissions SET reply = '$reply' WHERE id = '$id'") or die("SQL ERROR at ".__FILE__.":".__LINE__." => ".mysql_error());
mysql_query("UPDATE users_submissions SET denied = '$denied' WHERE id = '$id'") or die("SQL ERROR at ".__FILE__.":".__LINE__." => ".mysql_error());
mysql_query("UPDATE users_submissions SET reply_sent = '$reply_sent' WHERE id = '$id'") or die("SQL ERROR at ".__FILE__.":".__LINE__." => ".mysql_error());
header("Refresh:0");
echo '
									<div class="alert alert-success">
										<button type="button" class="close" data-dismiss="alert">×</button>
										<strong>Success!</strong> This users OOTW submission entry was updated. Redirecting you back to the dashboard...
									</div>';
