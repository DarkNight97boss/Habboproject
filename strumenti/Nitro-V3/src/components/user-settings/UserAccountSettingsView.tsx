...
const passwordStrength = (value: string): { score: number; label: string; color: string } =>
{
    ...
    if(score <= 1) return { score: 1, label: LocalizeText('useraccount.password.strength.weak'), color: 'bg-[#a81a12]' };
    if(score === 2) return { score: 2, label: LocalizeText('useraccount.password.strength.fair'), color: 'bg-[#ffc107]' };
    if(score === 3) return { score: 3, label: LocalizeText('useraccount.password.strength.good'), color: 'bg-[#1e7295]' };
    return { score: 4, label: LocalizeText('useraccount.password.strength.strong'), color: 'bg-[#00800b]' };
};
...
export const UserAccountSettingsView: FC<{}> = () =>
{
    ...
    const submitPasswordChange = async () =>
    {
        ...
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.all_fields_required') });
            return;
        }

        if(newPassword.length < MIN_PASSWORD_LENGTH)
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.password.min_length', [ 'min' ], [ MIN_PASSWORD_LENGTH.toString() ]) });
            return;
        }

        if(newPassword.length > MAX_PASSWORD_LENGTH)
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.password.too_long') });
            return;
        }

        if(newPassword !== confirmPassword)
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.password.mismatch') });
            return;
        }

        if(newPassword === currentPassword)
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.password.same_as_current') });
            return;
        }
        ...
        if(!token)
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.not_authenticated') });
            return;
        }
        ...
        try
        {
            ...
            if(!response.ok)
            {
                const message = typeof payload.error === 'string' && payload.error
                    ? payload.error
                    : LocalizeText('useraccount.error.request_failed', [ 'status' ], [ response.status.toString() ]);
            ...
            const message = typeof payload.message === 'string' && payload.message
                ? payload.message
                : LocalizeText('useraccount.success.password_updated');
        ...
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.server_unreachable') });
        }
        finally
        {
            setSubmitting(false);
    ...
    const submitEmailChange = async () =>
    {
        ...
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.email.too_long') });
            return;
        }

        if(!EMAIL_RE.test(newEmail))
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.email.invalid') });
            return;
        }
        ...
        try
        {
            ...
            const message = typeof payload.message === 'string' && payload.message
                ? payload.message
                : LocalizeText('useraccount.success.email_updated');
            setFeedback({ kind: 'success', message });
            setEmailCurrentPassword('');
            setNewEmail('');
            setShowEmailPassword(false);
    ...
    const submitUsernameChange = async () =>
    {
        ...
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.username.length', [ 'min', 'max' ], [ MIN_USERNAME_LENGTH.toString(), MAX_USERNAME_LENGTH.toString() ]) });
            return;
        }

        if(!USERNAME_RE.test(newUsername))
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.username.invalid_chars') });
            return;
        }

        if(newUsername === session.username)
        {
            setFeedback({ kind: 'error', message: LocalizeText('useraccount.error.username.same_as_current') });
            return;
        }
        ...
        try
        {
            ...
            const message = typeof payload.message === 'string' && payload.message
                ? payload.message
                : LocalizeText('useraccount.success.username_updated');
            setFeedback({ kind: 'success', message });
            setUsernameCurrentPassword('');
            setNewUsername('');
            setShowUsernamePassword(false);
    ...
    return (
        <NitroCardView className="user-account-settings-window w-[360px]" theme="primary-slim" uniqueKey="user-account-settings">
            <NitroCardHeaderView headerText={ LocalizeText('useraccount.title') } onCloseClick={ close } />
            ...
            <div className="relative flex items-center gap-3 px-3 py-2 bg-[linear-gradient(180deg,#2e8fb8_0%,#1e7295_100%)] text-white">
                ...
                <div className="relative flex flex-col leading-tight">
                    <Text small className="text-white/80 uppercase tracking-wider">{ LocalizeText('useraccount.my_account') }</Text>
                    <Text bold className="text-white text-[15px]">{ session.username || LocalizeText('useraccount.guest') }</Text>
                    <Text small className="text-white/80">{ LocalizeText('useraccount.header.subtitle') }</Text>
                </div>
            </div>
            ...
            <NitroCardContentView className="flex flex-col gap-2 text-black">
                { section === 'menu' && (
                    <div className="flex flex-col gap-2">
                        <Text small className="text-black/60 uppercase tracking-wider px-1">{ LocalizeText('useraccount.menu.account') }</Text>
                        <button
                            type="button"
                            ...
                            <div className="flex flex-col flex-1 leading-tight">
                                <Text bold>{ LocalizeText('useraccount.password.reset.title') }</Text>
                                <Text small className="text-black/60">{ LocalizeText('useraccount.password.reset.subtitle') }</Text>
                            </div>
                        ...
                        <button
                            ...
                            <div className="flex flex-col flex-1 leading-tight">
                                <Text bold>{ LocalizeText('useraccount.email.change.title') }</Text>
                                <Text small className="text-black/60">{ LocalizeText('useraccount.email.change.subtitle') }</Text>
                            </div>
                            <FaChevronRight className="text-black/40 group-hover:text-[#1e7295]" />
                        </button>
                        ...
                        <button
                            ...
                            <div className="flex flex-col flex-1 leading-tight">
                                <Text bold>{ LocalizeText('useraccount.username.change.title') }</Text>
                                <Text small className="text-black/60">{ LocalizeText('useraccount.username.change.subtitle') }</Text>
                            </div>
                        ...
                        <div className="flex items-center gap-3 rounded-md border border-dashed border-black/15 bg-black/[0.02] px-3 py-2 opacity-70">
                            ...
                            <div className="flex flex-col flex-1 leading-tight">
                                <Text bold className="text-black/60">{ LocalizeText('useraccount.more.title') }</Text>
                                <Text small className="text-black/50">{ LocalizeText('useraccount.more.subtitle') }</Text>
                            </div>
                        </div>
                    </div>
                ) }
                ...
                { section === 'password' && (
                    <div className="flex flex-col gap-2" onKeyDown={ (event: KeyboardEvent<HTMLDivElement>) => { if(event.key === 'Enter') { event.preventDefault(); submitPasswordChange(); } } }>
                        ...
                        <div className="flex items-start gap-2 rounded-md border border-[#1e7295]/30 bg-[#1e7295]/10 px-2 py-2 text-[11px] leading-4 text-[#0d3d52]">
                            ...
                            <span>{ LocalizeText('useraccount.password.hint.prefix') } <strong>{ LocalizeText('useraccount.password.hint.length', [ 'min' ], [ MIN_PASSWORD_LENGTH.toString() ]) }</strong>. { LocalizeText('useraccount.password.hint.suffix') }</span>
                        </div>

                        <label className="flex flex-col gap-1 text-[12px]">
                            <span className="font-bold">{ LocalizeText('useraccount.password.current.label') }</span>
                            <div className="relative flex items-center">
                                ...
                                <button
                                    type="button"
                                    aria-label={ showCurrent ? LocalizeText('useraccount.password.hide') : LocalizeText('useraccount.password.show') }
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            <span className="font-bold">{ LocalizeText('useraccount.password.new.label') }</span>
                            <div className="relative flex items-center">
                                <FaKey className="absolute left-2 text-black/40" size={ 12 } />
                                <input
                                ...
                                <button
                                    type="button"
                                    aria-label={ showNew ? LocalizeText('useraccount.password.hide') : LocalizeText('useraccount.password.show') }
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            <span className="font-bold">{ LocalizeText('useraccount.password.retype.label') }</span>
                            <div className="relative flex items-center">
                                <FaKey className="absolute left-2 text-black/40" size={ 12 } />
                                <input
                                    type={ showNew ? 'text' : 'password' }
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            ...
                            <Button variant="success" disabled={ submitting } onClick={ () => submitPasswordChange() }>
                                { submitting ? LocalizeText('useraccount.action.saving') : LocalizeText('useraccount.action.save_password') }
                            </Button>
                        </div>
                    </div>
                ) }
                ...
                { section === 'email' && (
                    <div className="flex flex-col gap-2" onKeyDown={ (event: KeyboardEvent<HTMLDivElement>) => { if(event.key === 'Enter') { event.preventDefault(); submitEmailChange(); } } }>
                        ...
                        <div className="flex items-start gap-2 rounded-md border border-[#1e7295]/30 bg-[#1e7295]/10 px-2 py-2 text-[11px] leading-4 text-[#0d3d52]">
                            <FaInfoCircle className="mt-[2px] shrink-0 text-[#1e7295]" />
                            <span>{ LocalizeText('useraccount.email.security.prefix') } <strong>{ LocalizeText('useraccount.password.current.label').toLowerCase() }</strong> { LocalizeText('useraccount.email.security.suffix') }</span>
                        </div>
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            ...
                            <div className="relative flex items-center">
                                ...
                                <button
                                    type="button"
                                    aria-label={ showEmailPassword ? LocalizeText('useraccount.password.hide') : LocalizeText('useraccount.password.show') }
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            <span className="font-bold">{ LocalizeText('useraccount.email.new.label') }</span>
                            <div className="relative flex items-center">
                                ...
                                <input
                                    ...
                                    disabled={ submitting }
                                    placeholder={ LocalizeText('useraccount.email.placeholder') }
                                />
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            ...
                            <Button variant="success" disabled={ submitting } onClick={ () => submitEmailChange() }>
                                { submitting ? LocalizeText('useraccount.action.saving') : LocalizeText('useraccount.action.save_email') }
                            </Button>
                        </div>
                    </div>
                ) }
                ...
                { section === 'username' && (
                    <div className="flex flex-col gap-2" onKeyDown={ (event: KeyboardEvent<HTMLDivElement>) => { if(event.key === 'Enter') { event.preventDefault(); submitUsernameChange(); } } }>
                        ...
                        <div className="flex items-start gap-2 rounded-md border border-[#ffc107]/50 bg-[#fff8e1] px-2 py-2 text-[11px] leading-4 text-[#5c4400]">
                            ...
                            <span>{ LocalizeText('useraccount.username.warning.prefix') } <strong>{ LocalizeText('useraccount.username.warning.bold') }</strong> { LocalizeText('useraccount.username.warning.suffix') }</span>
                        </div>
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            ...
                            <div className="relative flex items-center">
                                ...
                                <button
                                    type="button"
                                    aria-label={ showUsernamePassword ? LocalizeText('useraccount.password.hide') : LocalizeText('useraccount.password.show') }
                        ...
                        <label className="flex flex-col gap-1 text-[12px]">
                            <span className="font-bold">{ LocalizeText('useraccount.username.new.label') }</span>
                            <div className="relative flex items-center">
                                ...
                                <input
                                    ...
                                    disabled={ submitting }
                                    placeholder={ LocalizeText('useraccount.username.new.placeholder') }
                                />
                                { newUsername.length > 0 && USERNAME_RE.test(newUsername) && newUsername !== session.username && (
                                    <FaCheckCircle className="absolute right-2 text-[#00800b]" size={ 12 } />
                                ) }
                            </div>
                            <span className="text-[10px] text-black/50">{ LocalizeText('useraccount.username.help', [ 'min', 'max' ], [ MIN_USERNAME_LENGTH.toString(), MAX_USERNAME_LENGTH.toString() ]) }</span>
                        </label>
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                            ...
                            <Button variant="warning" disabled={ submitting } onClick={ () => submitUsernameChange() }>
                                { submitting ? LocalizeText('useraccount.action.renaming') : LocalizeText('useraccount.action.rename_me') }
                            </Button>
                        </div>
                    </div>
                ) }
