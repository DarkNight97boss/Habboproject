...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/games/tag/rollerskate/InteractionRollerskateField.class */
public class InteractionRollerskateField extends InteractionTagField {
    private final HashMap<Habbo, Integer> stepTimes;

    public InteractionRollerskateField(ResultSet resultSet, Item item) throws SQLException {
        super(resultSet, item, RollerskateGame.class);
        this.stepTimes = new HashMap<>();
    }

    public InteractionRollerskateField(int i, int i2, Item item, String str, int i3, int i4) {
        super(i, i2, item, str, i3, i4, RollerskateGame.class);
        this.stepTimes = new HashMap<>();
    }
    ...
    public void onWalkOff(RoomUnit roomUnit, Room room, Object[] objArr) throws Exception {
        ...
        AchievementManager.progressAchievement(habbo, Emulator.getGameEnvironment().getAchievementManager().getAchievement("RbTagC"), (Emulator.getIntUnixTimestamp() - this.stepTimes.get(habbo).intValue()) / 60);
    ...
    public void onPlace(Room room) {
        ...
        if (habbo != null) {
            AchievementManager.progressAchievement(habbo, Emulator.getGameEnvironment().getAchievementManager().getAchievement("RbTagA"));
        }
    }
}
