            ...
            if (userId != -1) {
                              ...
                              () -> {
                                 if (issue.state == ModToolTicketState.OPEN) {
                                    if (cfhTopic.action == CfhActionType.AUTO_IGNORE
                                       && this.client.getHabbo().getHabboStats().ignoreUser(this.client, reported.getHabboInfo().getId())) {
               ...
               if (cfhTopic != null && cfhTopic.action != CfhActionType.MODS) {
                  Emulator.getThreading().run(() -> {
                     if (issue.state == ModToolTicketState.OPEN) {
                        if (cfhTopic.action == CfhActionType.AUTO_IGNORE && this.client.getHabbo().getHabboStats().ignoreUser(this.client, issue.reportedId)) {
