...
public class GuildMembersComposer extends MessageComposer {
   ...
   public GuildMembersComposer(
      Guild guild, ArrayList<GuildMember> members, Habbo session, int pageId, int level, String searchValue, boolean isAdmin, int totalCount
   ) {
      this.guild = guild;
   ...
   protected ServerMessage composeInternal() {
      ...
      for (GuildMember member : this.members) {
         ...
         this.response.appendString(member.getRank().type < 3 ? cal.get(5) + "/" + (cal.get(2) + 1) + "/" + cal.get(1) : "");
      }
