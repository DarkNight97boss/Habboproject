...
public class ForumThreadComment implements Runnable, ISerialize {
   ...
   public static ForumThreadComment getById(int id) {
      ForumThreadComment foundComment = (ForumThreadComment)forumCommentsCache.get(id);
      ...
      try {
         ...
         try {
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM `guilds_forums_comments` WHERE `id` = ? LIMIT 1");
         ...
         label94: {
            try {
               PreparedStatement statement = connection.prepareStatement(
                  "INSERT INTO `guilds_forums_comments`(`thread_id`, `user_id`, `message`, `created_at`) VALUES (?, ?, ?, ?);", 1
               );
               ...
               label87: {
                  try {
                     ...
                     if (statement.executeUpdate() >= 1) {
                        ...
                        if (setx.next()) {
                           int commentId = setx.getInt(1);
                           createdComment = new ForumThreadComment(
                              commentId, thread.getThreadId(), poster.getHabboInfo().getId(), message, timestamp, ForumThreadState.OPEN, 0
                           );
   ...
   public void run() {
      if (this.needsUpdate) {
         try {
            ...
            try {
               PreparedStatement statement = connection.prepareStatement("UPDATE guilds_forums_comments SET `state` = ?, `admin_id` = ? WHERE `id` = ?");
