...
public class RequestCreateRoomEvent extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (!Emulator.getGameEnvironment().getRoomManager().layoutExists(modelName)) {
            LOGGER.error("[SCRIPTER] Nome layout non corretto \"{}\". {}", modelName, this.client.getHabbo().getHabboInfo().getUsername());
            return;
        }
        ...
        if (category == null || category.getMinRank() > this.client.getHabbo().getHabboInfo().getRank().getId()) {
            LOGGER.error("[SCRIPTER] Rank non corretto o ID categoria inesistente: \"{}\".{}", categoryId, this.client.getHabbo().getHabboInfo().getUsername());
            return;
        }
