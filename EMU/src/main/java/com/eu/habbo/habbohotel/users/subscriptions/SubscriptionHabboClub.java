...
public class SubscriptionHabboClub extends Subscription {
   ...
   public static ClubCenterDataComposer calculatePayday(HabboInfo habbo) {
      ...
      TObjectHashIterator queryParams = habbo.getHabboStats().subscriptions.iterator();

      while (queryParams.hasNext()) {
         Subscription sub = (Subscription)queryParams.next();
         if (sub.getSubscriptionType().equalsIgnoreCase("HABBO_CLUB")) {
      ...
      if (HC_PAYDAY_ENABLED && activeSub != null) {
         currentHcStreak = (int)Math.floor((Emulator.getIntUnixTimestamp() - activeSub.getTimestampStart()) / 86400.0);
         if (currentHcStreak < 1) {
            currentHcStreak = 0;
         }

         for (Entry<Integer, Integer> set : HC_PAYDAY_STREAK.entrySet()) {
            if (currentHcStreak >= set.getKey() && set.getValue() > creditRewardForStreakBonus) {
               creditRewardForStreakBonus = set.getValue();
            }
         }

         THashMap<String, Object> queryParamsx = new THashMap();
         queryParamsx.put("@user_id", habbo.getId());
         queryParamsx.put("@timestamp_start", habbo.getHabboStats().lastHCPayday);
         queryParamsx.put("@timestamp_end", HC_PAYDAY_NEXT_DATE);

         try {
            Connection connection = Emulator.getDatabase().getDataSource().getConnection();

            try {
               PreparedStatement statement = Database.preparedStatementWithParams(connection, HC_PAYDAY_QUERY, queryParamsx);
      ...
      try {
         ...
         try {
            PreparedStatement statement = connection.prepareStatement(
               "SELECT user_id FROM `users_subscriptions` WHERE subscription_type = 'HABBO_CLUB' AND `active` = 1 AND `timestamp_start` < ? AND (`timestamp_start` + `duration`) > ? GROUP BY user_id"
            );
            ...
            try {
               ...
               Date date = new Date(HC_PAYDAY_NEXT_DATE * 1000L);
               Date var24 = Emulator.modifyDate(date, HC_PAYDAY_INTERVAL);
               HC_PAYDAY_NEXT_DATE = (int)(var24.getTime() / 1000L);
               PreparedStatement stm2 = connection.prepareStatement("UPDATE `emulator_settings` SET `value` = ? WHERE `key` = ?");

               try {
                  SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                  stm2.setString(1, sdf.format(var24));
               ...
               stm2 = connection.prepareStatement(
                  "UPDATE users_settings SET last_hc_payday = ? WHERE user_id IN (SELECT user_id FROM `users_subscriptions` WHERE subscription_type = 'HABBO_CLUB' AND `active` = 1 AND `timestamp_start` < ? AND (`timestamp_start` + `duration`) > ? GROUP BY user_id)"
               );
   ...
   public static boolean claimPayDay(Habbo habbo, int amount, String currency) {
      ...
      habbo.alert(
         Emulator.getTexts()
            .getValue("subscriptions.hc.payday.message", "Woohoo HC Payday has arrived! You have received %amount% credits to your purse. Enjoy!")
