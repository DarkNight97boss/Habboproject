...
public class FreezeGame extends Game {
      ...
      if (this.state.equals(GameState.RUNNING) && habbo.getHabboInfo().isInGame() && habbo.getHabboInfo().getCurrentGame() == this.getClass()) {
         if (item.getExtradata().equalsIgnoreCase("0") || item.getExtradata().isEmpty()) {
            if (RoomLayout.tilesAdjecent(habbo.getRoomUnit().getCurrentLocation(), this.room.getLayout().getTile(item.getX(), item.getY()))
               && ((FreezeGamePlayer)habbo.getHabboInfo().getGamePlayer()).canThrowSnowball()) {
         ...
         if (this.room.getRoomSpecialTypes().hasFreezeExitTile()) {
            for (Habbo habbo : this.room.getHabbos()) {
               if (this.getTeamForHabbo(habbo) == null) {
                  TObjectHashIterator var3 = this.room.getItemsAt(habbo.getRoomUnit().getCurrentLocation()).iterator();
   ...
   public synchronized void run() {
      try {
         ...
         for (GameTeam team : this.teams.values()) {
            TObjectHashIterator totalScore = team.getMembers().iterator();

            while (totalScore.hasNext()) {
               GamePlayer player = (GamePlayer)totalScore.next();
               ((FreezeGamePlayer)player).cycle();
            }

            int totalScorex = team.getTotalScore();
            ...
            for (InteractionFreezeScoreboard scoreboard : scoreBoards.values()) {
               ...
               if (oldScore != totalScorex) {
                  scoreboard.setExtradata(totalScorex + "");
   ...
   public void stop() {
      ...
      for (GameTeam team : this.teams.values()) {
         ...
         TObjectHashIterator amount = players.iterator();

         while (amount.hasNext()) {
            GamePlayer p = (GamePlayer)amount.next();
            if (p.getScoreAchievementValue() > 0) {
               if (team.equals(winningTeam)) {
                  AchievementManager.progressAchievement(
                     p.getHabbo(), Emulator.getGameEnvironment().getAchievementManager().getAchievement("FreezeWinner"), p.getScoreAchievementValue()
                  );
      ...
      for (Entry<GameTeamColors, GameTeam> teamEntry : this.teams.entrySet()) {
         teamMemberCount.put(teamEntry.getKey(), teamEntry.getValue().getMembers().size());
      }

      for (Entry<Integer, InteractionFreezeGate> set : this.room.getRoomSpecialTypes().getFreezeGates().entrySet()) {
