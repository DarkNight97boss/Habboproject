...
public class InteractionGymEquipment extends InteractionEffectTile implements ICycleable {
   ...
   public boolean canWalkOn(RoomUnit roomUnit, Room room, Object[] objects) {
      return this.roomUnitId == -1
         ...
         && (roomUnit.getRoomUnitType().equals(RoomUnitType.USER) || roomUnit.getRoomUnitType().equals(RoomUnitType.BOT));
   }
            ...
            if (timestamp - this.startTime >= 120) {
               ...
               if (!achievement.isEmpty()) {
                  AchievementManager.progressAchievement(
                     habbo.getHabboInfo().getId(), Emulator.getGameEnvironment().getAchievementManager().getAchievement(achievement)
                  );
               }
   ...
   private RoomUnit getCurrentRoomUnit(Room room) {
      ...
      return bot != null ? bot.getRoomUnit() : null;
   }
}
