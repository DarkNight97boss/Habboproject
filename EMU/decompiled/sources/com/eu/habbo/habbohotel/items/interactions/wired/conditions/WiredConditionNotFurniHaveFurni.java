...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/conditions/WiredConditionNotFurniHaveFurni.class */
public class WiredConditionNotFurniHaveFurni extends InteractionWiredCondition {
    ...
    /* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/conditions/WiredConditionNotFurniHaveFurni$JsonData.class */
    static class JsonData {
        boolean all;
        List<Integer> itemIds;

        public JsonData(boolean z, List<Integer> list) {
            this.all = z;
            this.itemIds = list;
        }
    }

    public WiredConditionNotFurniHaveFurni(ResultSet resultSet, Item item) throws SQLException {
        super(resultSet, item);
        this.items = new THashSet<>();
    }

    public WiredConditionNotFurniHaveFurni(int i, int i2, Item item, String str, int i3, int i4) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] objArr) {
        ...
        return this.all ? this.items.stream().allMatch(habboItem -> {
            double z = habboItem.getZ() + Item.getCurrentHeight(habboItem);
            return room.getLayout().getTilesAt(room.getLayout().getTile(habboItem.getX(), habboItem.getY()), habboItem.getBaseItem().getWidth(), habboItem.getBaseItem().getLength(), habboItem.getRotation()).stream().noneMatch(roomTile -> {
                return room.getItemsAt(roomTile).stream().anyMatch(habboItem -> {
                    return habboItem != habboItem && habboItem.getZ() >= z;
                });
            });
        }) : this.items.stream().anyMatch(habboItem2 -> {
            double z = habboItem2.getZ() + Item.getCurrentHeight(habboItem2);
            return room.getLayout().getTilesAt(room.getLayout().getTile(habboItem2.getX(), habboItem2.getY()), habboItem2.getBaseItem().getWidth(), habboItem2.getBaseItem().getLength(), habboItem2.getRotation()).stream().noneMatch(roomTile -> {
                return room.getItemsAt(roomTile).stream().anyMatch(habboItem2 -> {
                    return habboItem2 != habboItem2 && habboItem2.getZ() >= z;
                });
            });
        });
    }
    ...
    public String getWiredData() {
        refresh();
        return WiredHandler.getGsonBuilder().create().toJson(new JsonData(this.all, (List) this.items.stream().map((v0) -> {
            return v0.getId();
    ...
    public void loadWiredData(ResultSet resultSet, Room room) throws SQLException {
        ...
        if (strArrSplit.length >= 1) {
            this.all = strArrSplit[0].equals("1");
    ...
    public void serializeWiredData(ServerMessage serverMessage, Room room) {
        ...
        while (it.hasNext()) {
            serverMessage.appendInt(Integer.valueOf(((HabboItem) it.next()).getId()));
        }
        ...
        serverMessage.appendInt(Integer.valueOf(this.all ? 1 : 0));
    ...
    public boolean saveData(WiredSettings wiredSettings) {
        ...
        this.all = wiredSettings.getIntParams()[0] == 1;
