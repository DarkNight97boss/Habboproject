...
public class InteractionFreezeBlock extends HabboItem {
   ...
   public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
      if (client != null) {
         ...
         TObjectHashIterator game = items.iterator();

         while (game.hasNext()) {
            HabboItem i = (HabboItem)game.next();
            if (i instanceof InteractionFreezeTile && (item == null || i.getZ() <= item.getZ())) {
               item = i;
            }
         }

         if (item != null) {
            FreezeGame gamex = (FreezeGame)room.getGame(FreezeGame.class);
            if (gamex == null) {
               return;
            }

            gamex.throwBall(client.getHabbo(), (InteractionFreezeTile)item);
         }
      }
   }
               ...
               if (player != null) {
                  ...
                  if (powerUp >= 2 && powerUp <= 7) {
                     ...
                     AchievementManager.progressAchievement(
                        player.getHabbo(), Emulator.getGameEnvironment().getAchievementManager().getAchievement("FreezePowerUp")
                     );
                  }
               }
            }
