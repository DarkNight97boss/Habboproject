...
public class PetPackageNameEvent extends MessageHandler {
   ...
   public void handle() throws Exception {
      ...
      if (room != null) {
         ...
         if (item != null && item.getUserId() == this.client.getHabbo().getHabboInfo().getId()) {
            if (!name.matches("^[a-zA-Z0-9]*$")) {
               this.client.sendResponse(new PetPackageNameValidationComposer(itemId, 3, name.replaceAll("^[a-zA-Z0-9]*$", "")));
               return;
            }
      ...
      this.client.sendResponse(new PetPackageNameValidationComposer(itemId, 0, ""));
   }
}
