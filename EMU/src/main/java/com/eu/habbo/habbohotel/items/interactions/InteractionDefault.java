...
public class InteractionDefault extends HabboItem {
   ...
   public void onMove(Room room, RoomTile oldLocation, RoomTile newLocation) {
      ...
      if (room.getItemsAt(oldLocation).stream().noneMatch(item -> item.getClass().isAssignableFrom(InteractionRoller.class))) {
         TObjectHashIterator var4 = room.getRoomUnits().iterator();

         while (var4.hasNext()) {
            RoomUnit unit = (RoomUnit)var4.next();
            if (oldLocation.unitIsOnFurniOnTile(unit, this.getBaseItem()) && !newLocation.unitIsOnFurniOnTile(unit, this.getBaseItem())) {
               try {
      ...
      if (room != null
         && (
            client == null
               ...
               || objects.length >= 2 && objects[1] instanceof WiredEffectType && objects[1] == WiredEffectType.TOGGLE_STATE
         )) {
         ...
         if (objects != null && objects.length > 0 && objects[0] instanceof Integer) {
            ...
            if (this.getBaseItem().getStateCount() > 0) {
               ...
               try {
                  ...
                  LOGGER.error(
                     "Incorrect extradata (" + this.getExtradata() + ") for item ID (" + this.getId() + ") of type (" + this.getBaseItem().getName() + ")"
                  );
               }
   ...
   public void onWalkOff(RoomUnit roomUnit, Room room, Object[] objects) throws Exception {
      ...
      if (roomUnit != null && (this.getBaseItem().getEffectF() > 0 || this.getBaseItem().getEffectM() > 0)) {
         ...
         if (objects != null && objects.length == 2 && objects[0] instanceof RoomTile && objects[1] instanceof RoomTile) {
            ...
            if (topItem != null
               && (
                  topItem.getBaseItem().getEffectM() == this.getBaseItem().getEffectM()
                     || topItem.getBaseItem().getEffectF() == this.getBaseItem().getEffectF()
               )) {
               return;
            }
   ...
   public boolean canToggle(Habbo habbo, Room room) {
      ...
      return rentSpace != null
         && RoomLayout.squareInSquare(
            ...
            RoomLayout.getRectangle(this.getX(), this.getY(), this.getBaseItem().getWidth(), this.getBaseItem().getLength(), this.getRotation())
         );
   }
