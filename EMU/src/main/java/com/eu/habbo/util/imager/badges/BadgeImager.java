...
public class BadgeImager {
   ...
   public synchronized boolean reload() {
      ...
      try {
         for (Entry<GuildPartType, THashMap<Integer, GuildPart>> set : Emulator.getGameEnvironment().getGuildManager().getGuildParts().entrySet()) {
            if (set.getKey() == GuildPartType.SYMBOL || set.getKey() == GuildPartType.BASE) {
               for (Entry<Integer, GuildPart> map : set.getValue().entrySet()) {
                     ...
                     try {
                        this.cachedImages
                           .put(
                              ...
                              ImageIO.read(
                                 new File(
                                    Emulator.getConfig().getValue("imager.location.badgeparts"), "badgepart_" + map.getValue().valueA.replace(".gif", ".png")
                                 )
                              )
                           );
                     } catch (Exception e) {
                        LOGGER.info(
                           "[Badge Imager] Missing Badge Part: "
                              + Emulator.getConfig().getValue("imager.location.badgeparts")
                              + "/badgepart_"
                              + map.getValue().valueA.replace(".gif", ".png")
                        );
                     }
                  }
                     ...
                     try {
                        this.cachedImages
                           .put(
                              ...
                              ImageIO.read(
                                 new File(
                                    Emulator.getConfig().getValue("imager.location.badgeparts"), "badgepart_" + map.getValue().valueB.replace(".gif", ".png")
                                 )
                              )
                           );
                     } catch (Exception e) {
                        LOGGER.info(
                           "[Badge Imager] Missing Badge Part: "
                              + Emulator.getConfig().getValue("imager.location.badgeparts")
                              + "/badgepart_"
                              + map.getValue().valueB.replace(".gif", ".png")
                        );
                     }
                  }
               }
   ...
   public void generate(Guild guild) {
      ...
      while (i < badge.length()) {
         ...
         for (int j = 0; j < 7; j++) {
            parts[count] = parts[count] + badge.charAt(i);
            i++;
         }
      }
      ...
      for (String s : parts) {
         if (!s.isEmpty()) {
            ...
            if (part != null) {
               BufferedImage imagePart = deepCopy((BufferedImage)this.cachedImages.get(part.valueA));
               if (imagePart != null) {
                  if (imagePart.getColorModel().getPixelSize() < 32) {
                     imagePart = convert32(imagePart);
                  }

                  Point point = getPoint(image, imagePart, position);
                  recolor(imagePart, colorFromHexString(color.valueA));
                  graphics.drawImage(imagePart, point.x, point.y, null);
               }

               if (!part.valueB.isEmpty()) {
                  imagePart = deepCopy((BufferedImage)this.cachedImages.get(part.valueB));
                  if (imagePart != null) {
                     if (imagePart.getColorModel().getPixelSize() < 32) {
                        imagePart = convert32(imagePart);
                     }
