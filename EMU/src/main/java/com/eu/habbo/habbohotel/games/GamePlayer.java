...
public class GamePlayer {
   ...
   public synchronized void addScore(int amount, boolean isWired) {
      if (this.habbo.getHabboInfo().getGamePlayer() != null
         ...
         && this.habbo.getHabboInfo().getCurrentRoom().getGame(this.habbo.getHabboInfo().getCurrentGame()).getTeamForHabbo(this.habbo) != null) {
         this.score += amount;
         if (this.score < 0) {
            this.score = 0;
         }
         ...
         WiredHandler.handle(
            WiredTriggerType.SCORE_ACHIEVED,
            this.habbo.getRoomUnit(),
            this.habbo.getHabboInfo().getCurrentRoom(),
            new Object[]{
               this.habbo.getHabboInfo().getCurrentRoom().getGame(this.habbo.getHabboInfo().getCurrentGame()).getTeamForHabbo(this.habbo).getTotalScore(),
               amount
            }
         );
      }
