            ...
            <div className="nitro-card-shell relative w-[420px] overflow-hidden shadow-lg" onClick={ e => e.stopPropagation() }>
                ...
                <div className="p-3 flex flex-col gap-2.5">
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        <input className={ inputClass } placeholder={ LocalizeText('catalog.admin.offer.name.placeholder') } type="text" value={ catalogName } onChange={ e => setCatalogName(e.target.value) } />
                    </div>
                    ...
                    <div className="bg-white rounded border-2 border-card-grid-item-border p-2.5">
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                            <div className="flex flex-col gap-0.5">
                                <label className="text-[9px] text-muted">{ LocalizeText('catalog.admin.offer.item_ids') }</label>
                                <input className={ inputClass } placeholder={ LocalizeText('catalog.admin.offer.item_ids.placeholder') } type="text" value={ itemIds } onChange={ e => setItemIds(e.target.value) } />
                            </div>
                            <div className="flex flex-col gap-0.5">
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                            ...
                            <div className="flex flex-col gap-0.5">
                                ...
                                <select className={ inputClass } value={ pointsType } onChange={ e => setPointsType(parseInt(e.target.value)) }>
                                    <option value={ 0 }>{ LocalizeText('catalog.admin.offer.points.type.duckets') }</option>
                                    <option value={ 5 }>{ LocalizeText('catalog.admin.offer.points.type.diamonds') }</option>
                                    <option value={ 101 }>{ LocalizeText('catalog.admin.offer.points.type.seasonal') }</option>
                                </select>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5 mb-1.5">
                            <div className="flex flex-col gap-0.5">
                                ...
                                <select className={ inputClass } value={ clubOnly } onChange={ e => setClubOnly(e.target.value) }>
                                    <option value="0">{ LocalizeText('generic.no') }</option>
                                    <option value="1">{ LocalizeText('generic.yes') }</option>
                                </select>
                            </div>
                            <div className="flex flex-col gap-0.5">
                                <label className="text-[9px] text-muted">{ LocalizeText('catalog.admin.offer.limited_stack') }</label>
                                <input className={ inputClass } min={ 0 } type="number" value={ limitedStack } onChange={ e => setLimitedStack(parseInt(e.target.value) || 0) } />
                            </div>
                            <div className="flex flex-col gap-0.5">
                                <label className="text-[9px] text-muted">{ LocalizeText('catalog.admin.offer.offer_id') }</label>
