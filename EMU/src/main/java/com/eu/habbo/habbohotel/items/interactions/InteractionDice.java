...
public class InteractionDice extends HabboItem {
   ...
   public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
      ...
      if (client != null
         && RoomLayout.tilesAdjecent(room.getLayout().getTile(this.getX(), this.getY()), client.getHabbo().getRoomUnit().getCurrentLocation())
         && !this.getExtradata().equalsIgnoreCase("-1")) {
         ...
         if (event.result > 0) {
            Emulator.getThreading().run(new RandomDiceNumber(room, this, event.result), 1500L);
         } else {
            Emulator.getThreading().run(new RandomDiceNumber(this, room, this.getBaseItem().getStateCount()), 1500L);
         }
      }
   }
