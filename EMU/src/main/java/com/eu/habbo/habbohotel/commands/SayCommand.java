...
public class SayCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      ...
      if (target.getHabboInfo().getCurrentRoom() == null) {
         gameClient.getHabbo()
            .whisper(Emulator.getTexts().getValue("commands.error.cmd_say.hotel_view").replace("%user%", params[1]), RoomChatMessageBubbles.ALERT);
         return true;
      }
      ...
      target.getHabboInfo()
         .getCurrentRoom()
         .sendComposer(new RoomUserTalkComposer(new RoomChatMessage(message.toString(), target, RoomChatMessageBubbles.NORMAL)).compose());
      gameClient.getHabbo()
         .whisper(
            Emulator.getTexts().getValue("commands.succes.cmd_say").replace("%user%", params[1]).replace("%message%", message.toString()),
