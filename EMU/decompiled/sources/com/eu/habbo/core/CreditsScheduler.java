...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/core/CreditsScheduler.class */
public class CreditsScheduler extends Scheduler {
    ...
    public void reloadConfig() {
        ...
        HC_MODIFIER = Emulator.getConfig().getDouble("hotel.auto.credits.hc_modifier", Double.valueOf(1.0d));
        if (this.disposed) {
            this.disposed = false;
            run();
        }
            ...
            if (value != null) {
                ...
                if (value.getHabboInfo().getCurrentRoom() != null || !IGNORE_HOTEL_VIEW) {
                    if (!value.getRoomUnit().isIdle() || !IGNORE_IDLED) {
                        value.giveCredits((int) (((double) value.getHabboInfo().getRank().getCreditsTimerAmount()) * (value.getHabboStats().hasActiveClub() ? HC_MODIFIER : 1.0d)));
                    }
                }
            }
        }
