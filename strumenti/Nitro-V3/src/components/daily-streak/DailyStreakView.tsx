...
export const DailyStreakView: FC<{}> = props =>
{
    ...
    useMessageEvent<DailyStreakClaimedEvent>(DailyStreakClaimedEvent, event =>
    {
        ...
        if(p.success)
        {
            ...
            const label = p.kind === KIND_CREDITS ? LocalizeText('dailystreak.reward.credits', [ 'amount' ], [ amount ])
                : p.kind === KIND_PIXELS ? LocalizeText('dailystreak.reward.duckets', [ 'amount' ], [ amount ])
                : p.kind === KIND_DIAMONDS ? LocalizeText('dailystreak.reward.diamonds', [ 'amount' ], [ amount ])
                : LocalizeText('dailystreak.reward.badge', [ 'badge' ], [ p.badgeCode ]);
            const milestone = p.milestoneAwarded ? LocalizeText('dailystreak.milestone.suffix', [ 'badge' ], [ p.milestoneBadge ]) : '';
            setMessage(`${label}${milestone}`);
        }
        else
        {
            setMessage(LocalizeText('dailystreak.already_claimed'));
        }
    });
    ...
    return (
        <NitroCardView uniqueKey="daily-streak" className="nitro-daily-streak" theme="primary-slim">
            <NitroCardHeaderView headerText={ LocalizeText('dailystreak.title') } onCloseClick={ () => setIsVisible(false) } />
            <div className="border-b border-zinc-200 p-2">
                <Flex alignItems="center" gap={ 2 }>
                    <FaFire className="text-danger" />
                    <Text bold>{ LocalizeText(currentStreak === 1 ? 'dailystreak.streak.one' : 'dailystreak.streak.many', [ 'count' ], [ String(currentStreak) ]) }</Text>
                    <Text small className="text-muted">{ LocalizeText('dailystreak.best', [ 'count' ], [ String(bestStreak) ]) }</Text>
                </Flex>
            </div>
                        ...
                        { grid.map(r =>
                        {
                            ...
                            return (
                                <Column key={ r.day } alignItems="center" justifyContent="center" gap={ 0 } style={ style }>
                                    <Text small bold>{ LocalizeText('dailystreak.day', [ 'day' ], [ String(r.day) ]) }</Text>
                    ...
                    <Flex justifyContent="end" gap={ 1 }>
                        { canClaim
                            ? <Button variant="success" disabled={ submitting } onClick={ submit }>
                                { submitting ? LocalizeText('dailystreak.claiming') : LocalizeText('dailystreak.claim') }
                              </Button>
                            : <Button variant="secondary" onClick={ () => setIsVisible(false) }>{ LocalizeText('dailystreak.close') }</Button>
                        }
                    </Flex>
                </Column>
