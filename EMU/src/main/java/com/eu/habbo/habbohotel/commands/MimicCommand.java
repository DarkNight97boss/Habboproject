...
public class MimicCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      if (params.length == 2) {
         ...
         if (habbo == null) {
            gameClient.getHabbo()
               .whisper(Emulator.getTexts().getValue("commands.error.cmd_mimic.not_found").replace("%user%", ""), RoomChatMessageBubbles.ALERT);
            return true;
            ...
            gameClient.getHabbo()
               .whisper(
                  Emulator.getTexts()
                     .getValue("commands.error.cmd_mimic.blocked")
            ...
            && FigureUtil.hasBlacklistedClothing(habbo.getHabboInfo().getLook(), gameClient.getHabbo().getForbiddenClothing())) {
            ...
            gameClient.getHabbo()
               ...
               .setLook(
                  ClothingValidationManager.VALIDATE_ON_MIMIC
                     ? ClothingValidationManager.validateLook(gameClient.getHabbo(), habbo.getHabboInfo().getLook(), habbo.getHabboInfo().getGender().name())
            ...
            gameClient.getHabbo()
               .whisper(
                  Emulator.getTexts()
                     .getValue("commands.succes.cmd_mimic.copied")
                     .replace("%user%", params[1])
                     .replace(
                        "%gender_name%",
                        habbo.getHabboInfo().getGender().equals(HabboGender.M)
