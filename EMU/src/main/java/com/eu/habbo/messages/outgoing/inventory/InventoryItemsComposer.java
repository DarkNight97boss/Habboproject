   ...
   public boolean execute(int a, HabboItem habboItem) {
      ...
      if (!habboItem.getBaseItem().getName().equals("floor")
         ...
         if (habboItem.getBaseItem().getName().equals("gnome_box")) {
            ...
            this.response
               .appendInt(
                  habboItem instanceof InteractionGift ? ((InteractionGift)habboItem).getColorId() * 1000 + ((InteractionGift)habboItem).getRibbonId() : 1
               );
         }
      ...
      if (habboItem.getBaseItem().getType() == FurnitureType.FLOOR) {
         ...
         this.response
            .appendInt(habboItem instanceof InteractionGift ? ((InteractionGift)habboItem).getColorId() * 1000 + ((InteractionGift)habboItem).getRibbonId() : 1);
      }
