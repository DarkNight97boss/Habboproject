...
public class WiredEffectMoveFurniAway extends InteractionWiredEffect {
         ...
         if (target != null) {
            if (target.getCurrentLocation().distance(t) <= 1.0) {
               ...
               if (newLocation != null
                  ...
                  && room.furnitureFitsAt(newLocation, item, item.getRotation(), true) == FurnitureMovementError.NONE
                  && room.moveFurniTo(item, newLocation, item.getRotation(), null, false) == FurnitureMovementError.NONE) {
   ...
   public String getWiredData() {
      return WiredHandler.getGsonBuilder()
         .create()
         .toJson(new WiredEffectMoveFurniAway.JsonData(this.getDelay(), this.items.stream().map(HabboItem::getId).collect(Collectors.toList())));
   }
   ...
   public void loadWiredData(ResultSet set, Room room) throws SQLException {
      ...
      if (wiredData.startsWith("{")) {
         WiredEffectMoveFurniAway.JsonData data = (WiredEffectMoveFurniAway.JsonData)WiredHandler.getGsonBuilder()
            .create()
            .fromJson(wiredData, WiredEffectMoveFurniAway.JsonData.class);
