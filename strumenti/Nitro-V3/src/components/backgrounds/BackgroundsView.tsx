...
import { GetOptionalConfigurationValue, LocalizeText } from '../../api';
...
}) =>
{
    ...
    return (
        <NitroCardView uniqueKey="backgrounds" className="absolute min-w-[535px] max-w-[535px] min-h-[389px] max-h-[389px]">
            <NitroCardHeaderView headerText={ LocalizeText('backgrounds.title') } onCloseClick={() => setIsVisible(false)} />
            <NitroCardTabsView>
                {TABS.map(tab => (
                    ...
                    >
                        { LocalizeText(`backgrounds.tab.${ tab }`) }
                    </NitroCardTabsItemView>
                ))}
            </NitroCardTabsView>
            <NitroCardContentView gap={1}>
                <Text bold center>{ LocalizeText('backgrounds.select_option') }</Text>
