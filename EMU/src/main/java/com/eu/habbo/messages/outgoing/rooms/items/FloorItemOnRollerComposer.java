...
public class FloorItemOnRollerComposer extends MessageComposer {
   ...
   public FloorItemOnRollerComposer(
      HabboItem item, HabboItem roller, RoomTile oldLocation, double oldZ, RoomTile newLocation, double newZ, double heightOffset, Room room
   ) {
      this.item = item;
      this.roller = roller;
   ...
   protected ServerMessage composeInternal() {
      ...
      if (this.oldLocation == null) {
         ...
         THashSet<RoomTile> tiles = this.room
            .getLayout()
            .getTilesAt(
               this.room.getLayout().getTile(oldX, oldY), this.item.getBaseItem().getWidth(), this.item.getBaseItem().getLength(), this.item.getRotation()
            );
         tiles.addAll(
            this.room
               .getLayout()
