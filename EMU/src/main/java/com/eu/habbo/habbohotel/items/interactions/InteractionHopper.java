...
public class InteractionHopper extends HabboItem {
      ...
      if (room != null) {
         ...
         if (loc != null) {
            if (this.canUseTeleport(client, loc, room)) {
               ...
               Emulator.getThreading().run(new HopperActionOne(this, room, client), 500L);
            } else {
   ...
   protected boolean canUseTeleport(GameClient client, RoomTile front, Room room) {
      if (client.getHabbo().getRoomUnit().getX() != front.x) {
         return false;
      } else if (client.getHabbo().getRoomUnit().getY() != front.y) {
         return false;
      } else if (client.getHabbo().getRoomUnit().isTeleporting) {
         return false;
      } else {
         return !room.getHabbosAt(this.getX(), this.getY()).isEmpty() ? false : this.getExtradata().equals("0");
      }
   }
}
