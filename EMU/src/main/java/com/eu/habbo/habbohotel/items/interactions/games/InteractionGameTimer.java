...
public class InteractionGameTimer extends HabboItem implements Runnable {
   ...
   public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
      ...
      if (objects.length >= 2 && objects[1] instanceof WiredEffectType) {
         ...
         this.isPaused = false;
         room.updateItem(this);
         WiredHandler.handle(WiredTriggerType.GAME_STARTS, null, room, new Object[0]);
         ...
         InteractionGameTimer.InteractionGameTimerAction state = InteractionGameTimer.InteractionGameTimerAction.START_STOP;
         if (objects.length >= 1 && objects[0] instanceof Integer) {
            state = InteractionGameTimer.InteractionGameTimerAction.getByAction((Integer)objects[0]);
         }
   ...
   public enum InteractionGameTimerAction {
      ...
      public static InteractionGameTimer.InteractionGameTimerAction getByAction(int action) {
         if (action == 1) {
            return START_STOP;
         } else {
