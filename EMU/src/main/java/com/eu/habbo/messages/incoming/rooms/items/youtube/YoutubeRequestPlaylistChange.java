         ...
         if (room != null) {
            if (room.isOwner(habbo) || habbo.hasPermission(Permission.ACC_ANYROOMOWNER)) {
               ...
               if (item != null && item instanceof InteractionYoutubeTV) {
                  Optional<YoutubeManager.YoutubePlaylist> playlist = Emulator.getGameEnvironment()
                     .getItemManager()
                     .getYoutubeManager()
                     .getPlaylistsForItemId(item.getBaseItem().getId())
                     .stream()
                  ...
                  if (playlist.isPresent()) {
                     ...
                     ((InteractionYoutubeTV)item).autoAdvance = Emulator.getThreading()
                        .run(new YoutubeAdvanceVideo((InteractionYoutubeTV)item), video.getDuration() * 1000);
