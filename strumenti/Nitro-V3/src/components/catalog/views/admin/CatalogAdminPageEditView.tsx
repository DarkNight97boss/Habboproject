...
export const CatalogAdminPageEditView: FC<{}> = () =>
{
    ...
    const runTranslate = async () =>
    {
        ...
        {
            setTranslateError(LocalizeText('catalog.admin.page.translate.error.nothing'));
            return;
        }

        if(!translateTargetLanguage)
        {
            setTranslateError(LocalizeText('catalog.admin.page.translate.error.no_language'));
            return;
        }
        ...
        {
            setTranslateError((error as Error)?.message || LocalizeText('catalog.admin.page.translate.error.failed'));
        }
        finally
        {
            setIsTranslating(false);
        ...
        <div className="bg-white rounded border-2 border-card-grid-item-border p-2.5 mb-2">
            ...
            <div className="grid grid-cols-3 gap-1.5">
                <div className="flex flex-col gap-0.5 col-span-2">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.caption') }</label>
                    <input className={ inputClass } value={ caption } onChange={ e => setCaption(e.target.value) } />
                </div>
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.min_rank') }</label>
                    <input className={ inputClass } min={ 1 } type="number" value={ minRank } onChange={ e => setMinRank(parseInt(e.target.value) || 1) } />
                </div>
                <div className="flex flex-col gap-0.5 col-span-2">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.caption_save') }</label>
                    <input className={ inputClass } value={ captionSave } onChange={ e => setCaptionSave(e.target.value) } />
                </div>
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.icon_image') }</label>
                    <input className={ inputClass } min={ 0 } type="number" value={ iconImage } onChange={ e => setIconImage(parseInt(e.target.value) || 0) } />
                </div>
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.mode') }</label>
                    <select className={ inputClass } value={ catalogMode } onChange={ e => setCatalogMode(e.target.value) }>
                        { MODE_OPTIONS.map(option => <option key={ option.value } value={ option.value }>{ option.label }</option>) }
                    </select>
                </div>
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.layout') }</label>
                ...
                <div className="flex flex-col gap-0.5">
                    <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.parent_id') }</label>
                ...
                <div className="flex flex-col gap-0.5 col-span-3">
                    <div className="flex items-center justify-between">
                        <label className="text-[9px] text-muted uppercase font-bold">{ LocalizeText('catalog.admin.page.page_text1') } <span className="text-muted normal-case font-normal opacity-70">{ LocalizeText('catalog.admin.page.page_text1.hint') }</span></label>
                        <button
                        ...
                        <div className="flex items-center gap-1 mb-1 p-1 bg-gray-50 border border-card-grid-item-border rounded">
                            <select
                                ...
                                { languagesLoading && !supportedLanguages.length &&
                                    <option value="">{ LocalizeText('catalog.admin.page.loading_languages') }</option> }
                            ...
                            <button
                                ...
                                { isTranslating ? <FaSpinner className="text-[8px] animate-spin" /> : LocalizeText('catalog.admin.page.translate.apply') }
                            </button>
                            <button
