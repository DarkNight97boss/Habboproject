package com.eu.habbo.plugin.events.furniture.wired;

import com.eu.habbo.habbohotel.items.interactions.InteractionWiredCondition;
import com.eu.habbo.habbohotel.items.interactions.InteractionWiredTrigger;
import com.eu.habbo.habbohotel.rooms.Room;
import com.eu.habbo.habbohotel.rooms.RoomUnit;
import com.eu.habbo.plugin.events.roomunit.RoomUnitEvent;

/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/plugin/events/furniture/wired/WiredConditionFailedEvent.class */
public class WiredConditionFailedEvent extends RoomUnitEvent {
    public final InteractionWiredTrigger trigger;
    public final InteractionWiredCondition condition;

    public WiredConditionFailedEvent(Room room, RoomUnit roomUnit, InteractionWiredTrigger interactionWiredTrigger, InteractionWiredCondition interactionWiredCondition) {
        super(room, roomUnit);
        this.trigger = interactionWiredTrigger;
        this.condition = interactionWiredCondition;
    }
}
