...
export const StaffMfaView: FC<{}> = props =>
{
    ...
    useMessageEvent<StaffMfaResultEvent>(StaffMfaResultEvent, event =>
    {
        ...
        {
            setCode('');
            setMessage(parser.message || LocalizeText('staffmfa.error.invalid'));
        }
    });
    ...
    const submit = () =>
    {
        ...
        if(!/^\d{6}$/.test(trimmed))
        {
            setMessage(LocalizeText('staffmfa.error.format'));
            return;
        }
    ...
    return (
        <NitroCardView uniqueKey="staff-mfa" className="nitro-staff-mfa" theme="primary-slim">
            <NitroCardHeaderView headerText={ LocalizeText('staffmfa.title') } onCloseClick={ () => setIsVisible(false) } />
            <NitroCardContentView className="text-black">
                <Column gap={ 2 }>
                    { !enrolled &&
                        <Column gap={ 1 }>
                            <Text bold>{ LocalizeText('staffmfa.setup.heading') }</Text>
                            <Text small>{ LocalizeText('staffmfa.setup.instructions') }</Text>
                            { !!otpauthUri &&
                            ...
                            <Text small className="text-muted">{ LocalizeText('staffmfa.setup.manual_hint') }</Text>
                    ...
                    { enrolled &&
                        <Text>{ LocalizeText('staffmfa.enrolled.prompt') }</Text> }
                    <input
                        autoFocus
                        type="text"
                        inputMode="numeric"
                    ...
                    <Flex justifyContent="end" gap={ 1 }>
                        <Button variant="success" disabled={ submitting || code.length !== 6 } onClick={ submit }>
                            { submitting ? LocalizeText('staffmfa.verifying') : LocalizeText('staffmfa.verify') }
                        </Button>
                    </Flex>
                </Column>
