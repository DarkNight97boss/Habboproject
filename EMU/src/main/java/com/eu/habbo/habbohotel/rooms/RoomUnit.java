...
public class RoomUnit {
   ...
   public boolean cycle(Room room) {
      try {
         ...
         if (rider != null) {
            ...
            if (rider.getRoomUnit().getCurrentLocation().x != this.getX() || rider.getRoomUnit().getCurrentLocation().y != this.getY()) {
               this.status
                  .put(
                     RoomUnitStatus.MOVE,
                     rider.getRoomUnit().getCurrentLocation().x
                        + ","
                        + rider.getRoomUnit().getCurrentLocation().y
                        + ","
                        + rider.getRoomUnit().getCurrentLocation().getStackHeight()
                  );
         ...
         } else if (!this.isWalking() && !this.isKicked && this.status.remove(RoomUnitStatus.MOVE) == null) {
            ...
            for (Entry<RoomUnitStatus, String> set : this.status.entrySet()) {
            ...
            if (this.path != null && !this.path.isEmpty()) {
               ...
               Habbo habboT = room.getHabbo(this);
               if (habboT != null && habboT.getHabboInfo().getRiding() != null) {
                  canfastwalk = false;
               }
               ...
               if (!room.tileWalkable(next)
                  || !RoomLayout.ALLOW_FALLING && height < -RoomLayout.MAXIMUM_STEP_HEIGHT
               ...
               if (!next.equals(this.goalLocation)
                  || next.state != RoomTileState.SIT
                  || overrideChecks
                  ...
                  if (habboItem != null
                     && (
                        habboItem != item
                           || !RoomLayout.pointInSquare(
                              habboItem.getX(),
                              habboItem.getY(),
                              habboItem.getX() + habboItem.getBaseItem().getWidth() - 1,
                              habboItem.getY() + habboItem.getBaseItem().getLength() - 1,
                              next.x,
                              next.y
                           )
                     )) {
                  ...
                  if (item != null) {
                     if (item == habboItem
                     ...
                     } else if (item.canWalkOn(this, room, null)) {
                        ...
                        if (habbo != null) {
                           ((ConditionalGate)item).onRejected(this, this.getRoom(), new Object[0]);
                        }
                  ...
                  if (habbo != null) {
                     ...
                     if (this.canLeaveRoomByDoor && isAtDoor && publicRoomKicks && !invalidated) {
                        Emulator.getThreading().run(new RoomUnitKick(habbo, room, false), 500L);
                     }
                  }
   ...
   public void findPath() {
      if (this.room != null
         ...
         && this.goalLocation != null
         && (this.goalLocation.isWalkable() || this.room.canSitOrLayAt(this.goalLocation.x, this.goalLocation.y) || this.canOverrideTile(this.goalLocation))) {
   ...
   public boolean canOverrideTile(RoomTile tile) {
      if (tile != null && this.room != null && this.room.getLayout() != null) {
         if (this.room.getItemsAt(tile).stream().anyMatch(i -> i.canOverrideTile(this, this.room, tile))) {
            return true;
         }

         int tileIndex = tile.x & 255 | tile.y << 12;
   ...
   public RoomTile getClosestAdjacentTile(short x, short y, boolean diagonal) {
      ...
      return this.getClosestTile(
         rotations.stream()
            ...
            .filter(t -> t != null && t.isWalkable() && (this.getCurrentLocation().equals(t) || !this.room.hasHabbosAt(t.x, t.y)))
