...
public class FreezeBotsCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      ...
      if (gameClient.getHabbo().getHabboInfo().getId() != gameClient.getHabbo().getHabboInfo().getCurrentRoom().getOwnerId()
         ...
         gameClient.getHabbo()
            .whisper(
               gameClient.getHabbo().getHabboInfo().getCurrentRoom().isAllowBotsWalk()
                  ? Emulator.getTexts().getValue("commands.succes.cmd_freeze_bots.unfrozen")
                  : Emulator.getTexts().getValue("commands.succes.cmd_freeze_bots.frozen"),
