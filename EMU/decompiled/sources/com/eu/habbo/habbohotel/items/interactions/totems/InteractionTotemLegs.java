...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/totems/InteractionTotemLegs.class */
public class InteractionTotemLegs extends InteractionDefault {
    public InteractionTotemLegs(ResultSet resultSet, Item item) throws SQLException {
        super(resultSet, item);
    }

    public InteractionTotemLegs(int i, int i2, Item item, String str, int i3, int i4) {
    ...
    public TotemType getTotemType() {
        ...
        return TotemType.fromInt((int) Math.ceil((i + 1) / 4.0f));
    }
    ...
    public TotemColor getTotemColor() {
        ...
        return TotemColor.fromInt(i - (4 * (getTotemType().type - 1)));
    }

    private void updateHead(Room room, RoomTile roomTile) {
        TObjectHashIterator it = room.getItemsAt(roomTile).iterator();
        while (it.hasNext()) {
            HabboItem habboItem = (HabboItem) it.next();
            if ((habboItem instanceof InteractionTotemHead) && habboItem.getZ() > getZ()) {
                ((InteractionTotemHead) habboItem).updateTotemState(room);
            }
        }
    }
    ...
    public void onClick(GameClient gameClient, Room room, Object[] objArr) throws Exception {
        ...
        updateHead(room, room.getLayout().getTile(getX(), getY()));
    }
