                    ...
                    <>
                        {isRoomMode && (
                            <div className="mb-2 p-2 bg-blue-900/50 rounded flex justify-between text-sm">
                                <span className="text-blue-300">
                                    📺 { LocalizeText('nitro.youtube.connected') }
                                </span>
                        ...
                        {broadcastVideo && broadcastSender && (
                            <div className="mt-2 p-2 bg-purple-900/50 rounded text-sm flex justify-between items-center">
                                <span className="text-purple-300">📡 { LocalizeText('nitro.youtube.broadcasting', [ 'sender' ], [ broadcastSender ]) }</span>
                                {isMyRoom && (
                                    <button
                                        onClick={() =>
                                        {
                        ...
                        <div className="mt-3 flex gap-2">
                            <input
                                ...
                                placeholder={ LocalizeText('youtube.player.input.placeholder') }
                            />
                            {isMyRoom && youtubeEnabled && videoId && (
                                <button
                                    ...
                                    title={ LocalizeText('nitro.youtube.broadcast.title') }
                                >
                    ...
                    <div className="space-y-2">
                        <div className="flex gap-2">
                            <input
                                ...
                                value={inputValue}
                                onChange={(e) => setInputValue(e.target.value)}
                                placeholder={ LocalizeText('youtube.playlist.add.placeholder') }
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                🔄 { LocalizeText('nitro.youtube.new_video') }
                            </button>
                            <button
                                onClick={() => setPlaylist([])}
                                className="px-3 py-2 bg-red-900 rounded text-white text-sm"
                            >
                                🗑 { LocalizeText('nitro.youtube.clear') }
                            </button>
                        </div>
                        {playlist.length === 0 ? (
                            <div className="p-4 text-center text-gray-500 text-sm">
                                { LocalizeText('nitro.youtube.playlist.empty') }
                            </div>
                        ) : (
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            <div className="text-gray-400 text-sm">
                                🕐 { LocalizeText('nitro.youtube.history') } ({history.length})
                            </div>
                            <button
                                onClick={() => setHistory([])}
                        ...
                        {history.length === 0 ? (
                            <div className="p-4 text-center text-gray-500 text-sm">
                                { LocalizeText('nitro.youtube.history.empty') }
                            </div>
                        ) : (
                    ...
                    <div className="space-y-3">
                        <div className="p-3 bg-gray-800 rounded">
                            <div className="text-gray-400 text-sm mb-2">
                                📤 { LocalizeText('nitro.youtube.share.video') }
                            </div>
                            {videoId ? (
                                <div className="space-y-2">
                                    <div className="flex gap-2">
                                ...
                                <div className="text-gray-500 text-sm text-center py-4">
                                    { LocalizeText('nitro.youtube.share.empty') }
                                </div>
                            )}
                        </div>
                        <div className="p-3 bg-gray-800 rounded">
                            <div className="text-gray-400 text-sm mb-2">
                                📋 { LocalizeText('nitro.youtube.quick_share') }
                            </div>
                    ...
                    return (
                        <div className="p-3 bg-gray-800 rounded">
                            <div className="flex justify-between items-center mb-2">
                                <div className="text-gray-400 text-sm">
                                📺 { watchers.length } { LocalizeText('nitro.youtube.watching') }
                                </div>
                                <button
                            ...
                            {watchers.length === 0 ? (
                                <div className="text-gray-500 text-sm text-center py-4">
                                { LocalizeText('nitro.youtube.no_watchers') }
                                </div>
                            ) : (
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                <label className="text-white text-sm">
                                    🔊 { LocalizeText('nitro.youtube.volume') }: { volume }%
                                </label>
                                <button
                                    onClick={() =>
                        ...
                        <div className="flex flex-wrap gap-3">
                            <label className="flex items-center gap-2 text-white text-sm cursor-pointer">
                                <input
                                    ...
                                    className="w-4 h-4"
                                />
                                🔇 { LocalizeText('nitro.youtube.mute') }
                            </label>
                            <label className="flex items-center gap-2 text-white text-sm cursor-pointer">
                                <input
                                    type="checkbox"
                                    ...
                                    className="w-4 h-4"
                                />
                                🔁 { LocalizeText('nitro.youtube.loop') }
                            </label>
                            <label className="flex items-center gap-2 text-white text-sm cursor-pointer">
                                <input
                                    type="checkbox"
                                    ...
                                    className="w-4 h-4"
                                />
                                🖥️ { LocalizeText('nitro.youtube.fullscreen') }
                            </label>
                        </div>

                        <div className="p-2 bg-gray-800 rounded text-xs text-gray-400">
                            <div className="font-bold mb-1">ℹ️ { LocalizeText('nitro.youtube.info') }</div>
                            <div>
                                📡 { LocalizeText('nitro.youtube.info.broadcast') }:{' '}
                                {broadcastVideo
                                    ? <span className="text-green-400">✓ { LocalizeText('nitro.youtube.info.broadcast.active', [ 'sender' ], [ broadcastSender ]) }</span>
                                    : <span className="text-gray-500">✕ { LocalizeText('nitro.youtube.info.no_video') }</span>}
                            </div>
                            <div>
                                🎮 { LocalizeText('nitro.youtube.info.control') }:{' '}
                                {isMyRoom
                                    ? <span className="text-green-400">✓ { LocalizeText('nitro.youtube.info.owner') }</span>
                                    : <span className="text-gray-500">✕ { LocalizeText('nitro.youtube.info.viewing_only') }</span>}
                            </div>
                            <div>
                                👁️ { LocalizeText('nitro.youtube.info.viewers') }:{' '}
