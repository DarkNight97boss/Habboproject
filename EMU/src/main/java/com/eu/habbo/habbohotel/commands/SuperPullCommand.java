...
public class SuperPullCommand extends Command {
   ...
   public boolean handle(GameClient gameClient, String[] params) throws Exception {
      if (params.length == 2) {
         ...
         if (habbo == null) {
            gameClient.getHabbo()
               .whisper(Emulator.getTexts().getValue("commands.error.cmd_pull.not_found").replace("%user%", params[1]), RoomChatMessageBubbles.ALERT);
            return true;
         }
         ...
         RoomTile tile = gameClient.getHabbo()
            .getHabboInfo()
            .getCurrentRoom()
            .getLayout()
         ...
         if (tile != null && tile.isWalkable()) {
            if (tile == gameClient.getHabbo().getHabboInfo().getCurrentRoom().getLayout().getDoorTile()) {
               gameClient.getHabbo()
                  .whisper(Emulator.getTexts().getValue("commands.error.cmd_pull.invalid").replace("%username%", params[1]), RoomChatMessageBubbles.ALERT);
               return true;
            }
            ...
            gameClient.getHabbo()
               ...
               .sendComposer(
                  new RoomUserTalkComposer(
                        new RoomChatMessage(
                           Emulator.getTexts()
                              .getValue("commands.succes.cmd_pull.pull")
