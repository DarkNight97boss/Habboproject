...
public class CatalogSelectClubGiftEvent extends MessageHandler {
   ...
   public void handle() throws Exception {
      ...
      if (itemName.isEmpty()) {
      ...
      } else if (this.client.getHabbo().getHabboStats().getRemainingClubGifts() < 1) {
         ...
         if (page == null) {
            ...
            CatalogItem catalogItem = page.getCatalogItems()
               .valueCollection()
               .stream()
               .filter(x -> x.getName().equalsIgnoreCase(itemName))
               .findAny()
               .orElse(null);
            if (catalogItem == null) {
               ...
               if (daysRequired > (int)Math.floor(this.client.getHabbo().getHabboStats().getPastTimeAsClub() / 86400.0)) {
                  ...
                  TObjectHashIterator var6 = catalogItem.getBaseItems().iterator();
