...
public class GuildInfoComposer extends MessageComposer {
   ...
   protected ServerMessage composeInternal() {
      boolean adminPermissions = this.client.getHabbo().getHabboStats().hasGuild(this.guild.getId())
            && this.client.getHabbo().hasPermission(Permission.ACC_GUILD_ADMIN)
         || Emulator.getGameEnvironment().getGuildManager().getOnlyAdmins(this.guild).get(this.client.getHabbo().getHabboInfo().getId()) != null;
      this.response.init(1702);
      ...
      this.response.appendBoolean(this.client.getHabbo().getHabboStats().guild == this.guild.getId());
      ...
      this.response.appendInt(!adminPermissions && this.guild.getOwnerId() != this.client.getHabbo().getHabboInfo().getId() ? 0 : this.guild.getRequestCount());
