   ...
   public void handle() throws Exception {
      ...
      if (guild != null && thread != null) {
         ...
         if (member == null) {
            ...
            if ((guild.canModForum() != SettingsState.OWNER || guild.getOwnerId() != this.client.getHabbo().getHabboInfo().getId() || isStaff)
               && (guild.canModForum() != SettingsState.ADMINS || isAdmin || isStaff)) {
               boolean pinChanged = isPinned != thread.isPinned();
               if (pinChanged) {
                  this.client
                     .sendResponse(
                        new BubbleAlertComposer(isPinned ? BubbleAlertKeys.FORUMS_THREAD_PINNED.key : BubbleAlertKeys.FORUMS_THREAD_UNPINNED.key).compose()
                     );
               }
               ...
               if (isLocked != thread.isLocked()) {
                  this.client
                     .sendResponse(
                        new BubbleAlertComposer(isLocked ? BubbleAlertKeys.FORUMS_THREAD_LOCKED.key : BubbleAlertKeys.FORUMS_THREAD_UNLOCKED.key).compose()
                     );
               }
