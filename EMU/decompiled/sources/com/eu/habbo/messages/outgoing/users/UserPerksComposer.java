...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/messages/outgoing/users/UserPerksComposer.class */
public class UserPerksComposer extends MessageComposer {
    ...
    protected ServerMessage composeInternal() {
        ...
        this.response.appendInt((Integer) 15);
        this.response.appendString("USE_GUIDE_TOOL");
        this.response.appendString("requirement.unfulfilled.helper_level_4");
        this.response.appendBoolean(Boolean.valueOf(Emulator.getGameEnvironment().getPermissionsManager().hasPermission(this.habbo, Permission.ACC_HELPER_USE_GUIDE_TOOL)));
        this.response.appendString("GIVE_GUIDE_TOURS");
        this.response.appendString(Emulator.PREVIEW);
        this.response.appendBoolean(Boolean.valueOf(Emulator.getGameEnvironment().getPermissionsManager().hasPermission(this.habbo, "acc_helper_give_guide_tours")));
        this.response.appendString("JUDGE_CHAT_REVIEWS");
        this.response.appendString("requirement.unfulfilled.helper_level_6");
        this.response.appendBoolean(Boolean.valueOf(Emulator.getGameEnvironment().getPermissionsManager().hasPermission(this.habbo, "acc_helper_judge_chat_reviews")));
        ...
        this.response.appendBoolean(Boolean.valueOf(this.habbo.getHabboStats().allowTrade()));
        this.response.appendString("HEIGHTMAP_EDITOR_BETA");
        this.response.appendString("requirement.unfulfilled.feature_disabled");
        this.response.appendBoolean(Boolean.valueOf(Emulator.getGameEnvironment().getPermissionsManager().hasPermission(this.habbo, Permission.ACC_FLOORPLAN_EDITOR)));
        ...
        this.response.appendString(Emulator.PREVIEW);
        this.response.appendBoolean(Boolean.valueOf(Emulator.getGameEnvironment().getPermissionsManager().hasPermission(this.habbo, "acc_camera")));
