<?php 
$getBan = mysql_query("SELECT * FROM `bans` INNER JOIN users ON users.id=users.id AND users.id =  '".$_SESSION['user']['id']."' WHERE `user_id` = '".$_SESSION['user']['id']."' OR `ip` = '".$_SERVER['REMOTE_ADDR']."'");
...
<html class="no-js?<?php echo time() ?>" lang="en">
	<head>
		<title>{shortname} ~ You're Banned!</title>
		<meta charset="UTF-8">
		...
		<meta property="og:title" content="{shortname} ~ You're Banned!">
		...
		<meta property="og:url" content="{url}/banned">
		...
		<meta name="twitter:title" content="{shortname} ~ You're Banned!">
		...
		<meta itemprop="name" content="{shortname} ~ You're Banned!">
		...
		<div class='uk-width1-1@s uk-width-1-3@m'>
			<center class='uk-margin'>
			 ...
			 <div class='habbo uk-card uk-card-default uk-card-body'>
				 <h3 class='uk-card-title chocolate__title__rainbow '><b>Hey {username}, Your Account is Banned!</b></h3>
					  <div class='uk-margin'>
						  <div class='uk-inline'>
							<center><i><b>Information Regarding Your Ban!</b></i>
							<hr></center>
						<b>Username:</b> <?php echo mysql_result(mysql_query("SELECT username FROM users WHERE id = '{$getInfo['user_id']}' LIMIT 1"), 0); ?><br/>
						<b>Ban Type:</b> <?php echo $getInfo['type']; ?><br/>
						<b>Ban Reason:</b> <?php echo $getInfo['ban_reason']; ?><br/>
						<b>Date Of Ban:</b>  <?php echo date("D, d F Y H:i (P)", $getInfo['timestamp']); ?><br/>
						<b>Ban Expiry Date:</b> <?php echo date("D, d F Y H:i (P)", $getInfo['ban_expire']); ?><br/>
						<b>Banned By:</b> <?php echo mysql_result(mysql_query("SELECT username FROM users WHERE id = '{$getInfo['user_staff_id']}' LIMIT 1"), 0); ?><br/>
		  ...
		  <div class='uk-width-1-2@m uk-visible@m'>
              <div class="uk-margin habbo habbo-opaque uk-card uk-card-default uk-card-body" style='padding-top: 35px; padding-bottom: 35px;margin-top: 115px'>
				...
				<dt><span uk-icon='icon: lifesaver'></span> <b>You've been banned from playing {shortname}!</b></dt>
				<dd>You've been banned from playing {shortname} for not following our rules, {shortname} is a game to enjoy and have fun not break the rules.
				<br/><br/>
				If you want to appeal your ban please <a href="{discord}"><b>click here</b></a>
				</dd>
			  </dl>
		  </div>
		</div>
