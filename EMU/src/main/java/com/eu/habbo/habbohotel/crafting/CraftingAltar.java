...
public class CraftingAltar {
   ...
   public Map<CraftingRecipe, Boolean> matchRecipes(Map<Item, Integer> amountMap) {
      ...
      for (Entry<Integer, CraftingRecipe> set : this.recipes.entrySet()) {
         boolean contains = true;
         if (!set.getValue().isLimited() || set.getValue().canBeCrafted()) {
            boolean equals = amountMap.size() == set.getValue().getIngredients().size();

            for (Entry<Item, Integer> entry : amountMap.entrySet()) {
               if (contains) {
                  if (set.getValue().getIngredients().containsKey(entry.getKey())) {
                     if (((Integer)set.getValue().getIngredients().get(entry.getKey())).equals(entry.getValue())) {
                        continue;
                     }

                     equals = false;
                     if ((Integer)set.getValue().getIngredients().get(entry.getKey()) > entry.getValue()) {
                        continue;
                     }
                  }
   ...
   public CraftingRecipe getRecipe(int id) {
      return (CraftingRecipe)this.recipes.get(id);
   }
   ...
   public CraftingRecipe getRecipe(Map<Item, Integer> items) {
      for (Entry<Integer, CraftingRecipe> set : this.recipes.entrySet()) {
         ...
         for (Entry<Item, Integer> entry : recipe.getIngredients().entrySet()) {
