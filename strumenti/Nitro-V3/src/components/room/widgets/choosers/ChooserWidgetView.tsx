...
export const ChooserWidgetView: FC<ChooserWidgetViewProps> = props =>
{
    ...
    {
        const names = Array.from(new Set(items.map(item => item.ownerName || LocalizeText('widget.chooser.owner.unknown'))));
        return names.sort();
    }, [ items ]);
                    ...
                    { pickallFurni && (
                        ...
                        >
                            ...
                            { ownerNames.length > 0 ? (
                                ...
                                <option disabled>{ LocalizeText('widget.chooser.owner.none_found') }</option>
                            )}
                        </select>
                    )}
                </Flex>
                ...
                <InfiniteScroll rows={ filteredItems } rowRender={ row => (
                    ...
                    >
                        ...
                        <Text truncate>
                            ...
                            { type === 'furni' && row.ownerName && row.ownerName !== '-' && ` (${ LocalizeText('widget.chooser.owner.label') }: ${row.ownerName})` }
                        </Text>
                    </Flex>
                )} />
                { pickallFurni && (
