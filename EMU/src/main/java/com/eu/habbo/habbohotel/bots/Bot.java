...
public class Bot implements Runnable {
         ...
         try {
            ...
            try {
               ...
               try {
                  ...
                  statement.setDouble(9, this.roomUnit == null ? 0.0 : this.roomUnit.getZ());
   ...
   public void cycle(boolean allowBotsWalk) {
      if (this.roomUnit != null) {
         if (allowBotsWalk
            && this.canWalk
            && !this.roomUnit.isWalking()
            && this.roomUnit.getWalkTimeOut() < Emulator.getIntUnixTimestamp()
         ...
         if (!this.chatLines.isEmpty() && this.chatTimeOut <= Emulator.getIntUnixTimestamp() && this.chatAuto && this.room != null) {
            this.lastChatIndex = this.chatRandom
               ? (short)Emulator.getRandom().nextInt(this.chatLines.size())
               : (this.lastChatIndex == this.chatLines.size() - 1 ? 0 : this.lastChatIndex++);
            ...
            if (!WiredHandler.handle(WiredTriggerType.SAY_SOMETHING, this.getRoomUnit(), this.room, new Object[]{message})) {
               this.talk(message);
            }
   ...
   public void talk(String message) {
      if (this.room != null) {
         ...
         this.room
            .botChat(
               new RoomUserTalkComposer(new RoomChatMessage(event.message, this.roomUnit, RoomChatMessageBubbles.getBubble(this.getBubbleId()))).compose()
            );
   ...
   public void shout(String message) {
      if (this.room != null) {
         ...
         this.room
            .botChat(
               new RoomUserShoutComposer(new RoomChatMessage(event.message, this.roomUnit, RoomChatMessageBubbles.getBubble(this.getBubbleId()))).compose()
            );
         if (message.equals("o/") || message.equals("_o/")) {
            this.room.sendComposer(new RoomUserActionComposer(this.roomUnit, RoomUserAction.WAVE).compose());
         }
   ...
   public void whisper(String message, Habbo habbo) {
      if (this.room != null && habbo != null) {
         ...
         event.target
            .getClient()
            .sendResponse(new RoomUserWhisperComposer(new RoomChatMessage(event.message, this.roomUnit, RoomChatMessageBubbles.getBubble(this.getBubbleId()))));
      }
   }
