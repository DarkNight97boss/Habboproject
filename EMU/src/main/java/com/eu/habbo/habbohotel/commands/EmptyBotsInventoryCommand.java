      ...
      if (params.length != 1 && (params.length < 2 || params[1].equals(Emulator.getTexts().getValue("generic.yes")))) {
         if (params.length >= 2 && params[1].equalsIgnoreCase(Emulator.getTexts().getValue("generic.yes"))) {
            ...
            if (habbo != null) {
               ...
               gameClient.getHabbo()
                  .whisper(
                     Emulator.getTexts().getValue("commands.succes.cmd_empty_bots.cleared").replace("%username%", habbo.getHabboInfo().getUsername()),
         ...
         if (gameClient.getHabbo().getHabboInfo().getCurrentRoom() != null) {
            if (gameClient.getHabbo().getHabboInfo().getCurrentRoom().getUserCount() > 10) {
               gameClient.getHabbo()
                  .alert(
                     Emulator.getTexts()
                        .getValue("commands.succes.cmd_empty_bots.verify")
