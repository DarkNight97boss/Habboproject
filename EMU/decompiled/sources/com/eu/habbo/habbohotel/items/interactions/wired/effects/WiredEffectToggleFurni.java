...
/* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/effects/WiredEffectToggleFurni.class */
public class WiredEffectToggleFurni extends InteractionWiredEffect {
    ...
    private static final List<Class<? extends HabboItem>> FORBIDDEN_TYPES = new ArrayList<Class<? extends HabboItem>>() { // from class: com.eu.habbo.habbohotel.items.interactions.wired.effects.WiredEffectToggleFurni.1
        {
    ...
    /* JADX INFO: loaded from: Habbo-3.5.3.jar:com/eu/habbo/habbohotel/items/interactions/wired/effects/WiredEffectToggleFurni$JsonData.class */
    ...
    public WiredEffectToggleFurni(ResultSet resultSet, Item item) throws SQLException {
        super(resultSet, item);
        this.items = new THashSet<>();
    }

    public WiredEffectToggleFurni(int i, int i2, Item item, String str, int i3, int i4) {
    ...
    public void serializeWiredData(ServerMessage serverMessage, Room room) {
        ...
        room.getRoomSpecialTypes().getTriggers(getX(), getY()).forEach(new TObjectProcedure<InteractionWiredTrigger>() { // from class: com.eu.habbo.habbohotel.items.interactions.wired.effects.WiredEffectToggleFurni.2
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] objArr) {
        ...
        while (it.hasNext()) {
            HabboItem habboItem = (HabboItem) it.next();
            if (habboItem == null || habboItem.getRoomId() == 0 || FORBIDDEN_TYPES.stream().anyMatch(cls -> {
                return cls.isAssignableFrom(habboItem.getClass());
            })) {
                tHashSet.add(habboItem);
            } else {
                try {
                    if (habboItem.getBaseItem().getStateCount() > 1 || (habboItem instanceof InteractionGameTimer)) {
                        int i = 0;
                        if (!habboItem.getExtradata().isEmpty()) {
                            try {
                                i = Integer.parseInt(habboItem.getExtradata());
                            } catch (NumberFormatException e) {
                            }
                        }
                        habboItem.onClick((habbo == null || (habboItem instanceof InteractionGameTimer)) ? null : habbo.getClient(), room, new Object[]{Integer.valueOf(i), getType()});
                    }
                } catch (Exception e2) {
    ...
    public void loadWiredData(ResultSet resultSet, Room room) throws SQLException {
        ...
        if (string.startsWith("{")) {
            ...
            while (it.hasNext()) {
                ...
                if (!(habboItem instanceof InteractionFreezeBlock) && !(habboItem instanceof InteractionFreezeTile) && !(habboItem instanceof InteractionCrackable) && habboItem != null) {
        ...
        if (strArrSplit.length == 2 && strArrSplit[1].contains(";")) {
            for (String str : strArrSplit[1].split(";")) {
                ...
                if (!(habboItem2 instanceof InteractionFreezeBlock) && !(habboItem2 instanceof InteractionFreezeTile) && !(habboItem2 instanceof InteractionCrackable) && habboItem2 != null) {
