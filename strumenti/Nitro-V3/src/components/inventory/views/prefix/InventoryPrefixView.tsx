...
}> = ({ iconKey, displayName, isSelected, isActive, onClick }) =>
{
    return (
        <div
            ...
            onClick={ onClick }>
            { isActive && <span className="absolute right-1 top-1 rounded bg-[#15954c] px-1 py-0.5 text-[8px] font-bold uppercase text-white">{ LocalizeText('inventory.prefix.badge.active') }</span> }
            ...
            <div className="shrink-0 rounded border border-black/10 bg-[#C9C9C9] p-1">
                <div className="flex items-center gap-2">
                    <button
                        ...
                        { LocalizeText('inventory.prefix.tab.prefixes') }
                    </button>
                    <button
                        ...
                        { LocalizeText('inventory.prefix.tab.icons') }
                    </button>
                </div>
            </div>
                    ...
                    <div className="col-span-5 flex flex-col justify-between overflow-auto">
                        { activePrefix &&
                            <div className="flex flex-col gap-1">
                                <span className="min-h-[1.25rem] truncate text-sm leading-5">{ LocalizeText('inventory.prefix.active') }</span>
                        ...
                        { !activePrefix &&
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="flex items-center justify-center rounded-md border-2 border-dashed border-card-grid-item-border bg-card-grid-item p-3 opacity-50">
                                    <span className="text-sm">{ LocalizeText('inventory.prefix.none_active') }</span>
                                </div>
                            </div> }
                        { !!selectedPrefix &&
                            <div className="mt-2 flex flex-col gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                    <NitroButton
                                        ...
                                        { selectedPrefix.active ? LocalizeText('inventory.prefix.action.deactivate') : LocalizeText('inventory.prefix.action.activate') }
                                    </NitroButton>
            ...
            { activeTab === 'icons' &&
                <div className="grid h-full grid-cols-12 gap-2">
                    <div className="col-span-7 flex flex-col gap-1 overflow-auto pr-1">
                        ...
                        { !hasNickIcons &&
                            <div className="flex h-full items-center justify-center text-sm opacity-50">
                                { LocalizeText('inventory.icon.none_purchased') }
                            </div> }
                    </div>
                    <div className="col-span-5 flex flex-col justify-between overflow-auto">
                        <div className="flex flex-col gap-1">
                            <span className="min-h-[1.25rem] truncate text-sm leading-5">{ LocalizeText('inventory.icon.active') }</span>
                            <div className={ `flex min-h-[88px] items-center justify-center rounded-md border-2 bg-card-grid-item p-3 ${ activeNickIcon ? 'border-green-400' : 'border-dashed border-card-grid-item-border opacity-50' }` }>
                                ...
                                { !activeNickIcon && <span className="text-sm">{ LocalizeText('inventory.icon.none_active') }</span> }
                            </div>
                        </div>
                        { !!selectedNickIcon &&
                            <div className="mt-2 flex flex-col gap-2">
                                ...
                                <Button disabled={ false } onClick={ () => selectedNickIcon.active ? deactivateNickIcon() : activateNickIcon(selectedNickIcon.id) }>
                                    { selectedNickIcon.active ? LocalizeText('inventory.prefix.action.deactivate') : LocalizeText('inventory.prefix.action.activate') }
                                </Button>
                            </div> }
                    </div>
                </div> }
